...
# Finding the start of the checkout block
match = re.search(r'isCheckoutOpen\s*&&\s*\(', fc)
if not match:
    # Try alternative search for the line before
    match = re.search(r'\{\s+isCheckoutOpen\s*&&\s*\(', fc)

if not match:
    print("Start marker still not found")
    # Print some lines around where it should be
    lines = fc.splitlines()
    for i in range(1390, min(1420, len(lines))):
        print(f"{i+1}: {lines[i]}")
    exit(1)

# Backtrack to the nearest {
start_idx = fc.rfind('{', 0, match.start())
                                    ...
                                    {invoiceResult?.detail && (
                                        <div className="bg-emerald-500/5 p-8 rounded-[2.5rem] border border-emerald-500/20 w-full space-y-4 shadow-2xl relative overflow-hidden">
                                            <div className="flex items-center justify-between text-left">
                                                <span className="text-[9px] font-black text-emerald-500/70 uppercase tracking-[0.3em]">Clave de Acceso SRI/SUNAT</span>
...
print("Modal refactored successfully with flexible regex")
