...
-- Primero verificamos si la tabla existe y agregamos columnas
DO $$ 
BEGIN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'asesores_empresa' AND polrelid = 'asesores'::regclass) THEN
    CREATE POLICY "asesores_empresa" ON asesores
      FOR ALL USING (empresa_id = auth.jwt() ->> 'empresa_id');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'campanas_empresa' AND polrelid = 'campanas'::regclass) THEN
    CREATE POLICY "campanas_empresa" ON campanas
      FOR ALL USING (empresa_id = auth.jwt() ->> 'empresa_id');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policy WHERE polname = 'integraciones_empresa' AND polrelid = 'integraciones'::regclass) THEN
