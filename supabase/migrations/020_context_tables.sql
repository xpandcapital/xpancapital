...
INSERT INTO producto_estados (empresa_id, nombre, slug, color, orden, es_default)
VALUES 
...
INSERT INTO unidades_medida (empresa_id, nombre, abreviatura, tipo, orden)
VALUES 
...
INSERT INTO sku_patrones (empresa_id, nombre, prefijo, orden)
VALUES 
    ('6186f014-c8c7-4027-9f08-8acf2bae3eae', 'Promoción Especial', 'PRO', 0),
    ('6186f014-c8c7-4027-9f08-8acf2bae3eae', 'Liquidación', 'LIQ', 1),
    ('6186f014-c8c7-4027-9f08-8acf2bae3eae', 'Exclusivo', 'EXC', 2)
ON CONFLICT (empresa_id, prefijo) DO NOTHING;

-- ═══════════════════════════════════════════════════════════════════════════════
-- 4. EXTENDER EMPRESA_CONFIG
-- ═══════════════════════════════════════════════════════════════════════════════
...
INSERT INTO envio_zonas (empresa_id, nombre, regiones, precio_base, precio_por_gramo, dias_estimados, orden)
VALUES 
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 6. MONEDAS_CONFIG Y TASAS_CAMBIO
-- ═══════════════════════════════════════════════════════════════════════════════
...
INSERT INTO monedas_config (empresa_id, moneda_base, monedas_activas)
VALUES ('6186f014-c8c7-4027-9f08-8acf2bae3eae', 'USD', ARRAY['USD', 'PEN', 'MXN', 'EUR'])
ON CONFLICT (empresa_id) DO NOTHING;

-- ═══════════════════════════════════════════════════════════════════════════════
-- 7. CARRITOS Y CARRITO_ITEMS
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 8. LANDING_SECCIONES Y USUARIO_PREFERENCIAS
-- ═══════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS favoritos (
    ...
    UNIQUE(empresa_id, user_id, producto_id)
);

CREATE INDEX IF NOT EXISTS idx_favoritos_usuario ON favoritos(user_id);
...
CREATE POLICY "Usuarios ven sus favoritos" ON favoritos FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Usuarios modifican sus favoritos" ON favoritos FOR ALL USING (auth.uid() = user_id);
