...
// 1. Destructuring fix
if (!code.includes('shippingCost, setShippingCost')) {
    code = code.replace(
...
// 2. CustomDatePicker unclosed div
const dpPattern = "        </div>\n    );\n};";
if (code.includes(dpPattern)) {
    code = code.replace(dpPattern, "        </div>\n        </div>\n    );\n};");
}

// 3. Olva Courier button
code = code.replace(/const shippingCost = prompt\('Ingrese costo de envío Olva \(S\/\):', '15'\);[\s\S]*?alert\('Costo de envío sumado al ajuste de total'\);\s+\}/,
`const cost = prompt(\`Ingrese costo de envío Olva / Courier (\${currency}):\`, '15');
                                    if (cost && !isNaN(parseFloat(cost))) {
                                        setShippingCost(parseFloat(cost));
                                        alert('Costo de envío actualizado');
                                    }`);

// 4. Totals (Shipping + Tax)
if (!code.includes('Envío Olva Courier')) {
...
// 5. WhatsApp
code = code.replace(/<button className="flex items-center gap-2 bg-emerald-500\/10 border border-emerald-500\/30 px-3 py-1.5 rounded-full ml-auto group hover:bg-emerald-500\/20 transition-all">[\s\S]*?Enviar por WhatsApp[\s\S]*?<\/button>/,
`<button 
                                            onClick={() => {
...
// 6. Split and Balance UI
const modStart = code.indexOf('isCheckoutOpen &&');
let ui = code.substring(0, modStart).trimEnd();
if (ui.endsWith('{')) ui = ui.substring(0, ui.length-1).trimEnd();

// Close UI divs (should be 7 based on my count)
...
fs.writeFileSync('components/superadmin/POSManager.tsx', ui + modal);
console.log('FINAL RECONSTRUCTION DONE');
