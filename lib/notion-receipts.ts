...
const getPropValue = (props: NotionPageProperties, name: string): unknown => {
  const prop: NotionPropertyValue | undefined = props[name];
  if (!prop) return null;

  switch (prop.type) {
    case 'title': return prop.title?.map((t: NotionRichTextItem) => t.plain_text).join('') || '';
    case 'rich_text': return prop.rich_text?.map((t: NotionRichTextItem) => t.plain_text).join('') || '';
    case 'number': return prop.number ?? null;
    case 'select': return prop.select?.name || null;
    case 'multi_select': return prop.multi_select?.map((s: NotionMultiSelectProperty) => s.name) || [];
    case 'date': return prop.date?.start || null;
    case 'checkbox': return prop.checkbox ?? false;
    case 'files': return prop.files?.map((f: NotionFileProperty) => f.file?.url || f.external?.url).filter(Boolean) || [];
    case 'relation': return prop.relation?.map((r: NotionRelationProperty) => r.id) || [];
    case 'formula': return prop.formula?.string || prop.formula?.number || prop.formula?.boolean || null;
    case 'rollup':
      if (prop.rollup?.type === 'number') return prop.rollup.number;
      if (prop.rollup?.type === 'array') return prop.rollup.array?.map((a: { type: string; title?: NotionRichTextItem[]; number?: number; string?: string }) => {
...
const findPropByKeywords = (props: NotionPageProperties, keywords: string[]): unknown => {
...
export function mapNotionPageToReceipt(page: NotionPage): NotionReceipt {
  ...
  const titleProp = Object.values(props).find((p: NotionPropertyValue) => p.type === 'title');
  ...
  const tipoStr = String(get('Tipo') || get('Tipo de pago') || get('Categoría') || get('Categoria') || 'otro').toLowerCase();
  ...
  return {
    ...
    lot_notion_id: lotNotionId ? String(lotNotionId) : null,
    lot_number: parsedLotNumber || '',
    receipt_number: receiptNumber,
    amount: monto || 0,
    date: fecha ? String(fecha) : null,
    receipt_type: receiptType,
    concept: conceptoFinal,
    payment_method: metodoPago ? String(metodoPago) : null,
    reference: referencia ? String(referencia) : null,
    file_url: Array.isArray(archivos) ? archivos[0] : (archivos ? String(archivos) : null),
...
export async function syncNotionReceipts(databaseId: string, projectId: string): Promise<SyncReceiptsResult> {
  const errors: string[] = [];
  ...
  try {
    ...
    } catch { /* la resolución falló, usar ID original */ }
    ...
    for (const page of pages) {
      try {
        ...
        errors.push(`Error procesando recibo ${page.id}: ${message}`);
      }
    }
    ...
    return { success: false, receipts: [], errors: [message] };
  }
}
