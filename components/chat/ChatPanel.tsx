...
export function ChatPanel({ onClose, onMinimize }: ChatPanelProps) {
  ...
  const cargarContactos = useCallback(async () => {
    ...
    try {
      ...
      if (error) console.error("[ChatPanel] Error cargando contactos:", error);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "chat_llamadas", filter: `recibida_por=eq.${user.id}` }, (payload) => {
        const llamada = payload.new as any;
        if (llamada.estado === "llamando") setLlamadaEntranteId(llamada.id);
      })
      .subscribe();
      ...
      if (salaActiva.tipo === "ia") {
        try {
          ...
          await fetch("/api/chat/ai", {
            ...
            body: JSON.stringify({ mensaje: contenido, sala_id: salaActiva.id, contexto, empresa_id: user?.empresa_id }),
          });
        } catch (err) {
  ...
  const handleInputChange = (value: string) => {
    ...
    if (salaActiva && value.length > 0) setEscribiendo(salaActiva.id, true);
  };
  ...
  const renderMensaje = (msg: ChatMensaje | VisitorMensaje, index: number) => {
    ...
    return (
      <motion.div
        ...
        className={`flex gap-3 mb-3 ${esMio ? "flex-row-reverse" : "flex-row"}`}
      >
          ...
          >
            ...
            {(msg as ChatMensaje).tipo === "imagen" && (msg as ChatMensaje).archivo_url ? (
              <a href={(msg as ChatMensaje).archivo_url!} target="_blank" rel="noopener noreferrer">
                <img src={(msg as ChatMensaje).archivo_url!} alt={(msg as ChatMensaje).archivo_nombre || "Imagen"} className="max-w-[200px] max-h-[200px] rounded-lg object-cover" />
              </a>
            ) : (msg as ChatMensaje).tipo === "archivo" && (msg as ChatMensaje).archivo_url ? (
              <a href={(msg as ChatMensaje).archivo_url!} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 underline">
              ...
              <div className="absolute -top-2 -right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <Popover open={menuMensaje === msg.id} onOpenChange={(open) => setMenuMensaje(open ? msg.id : null)}>
                  ...
                  <PopoverContent className="w-32 p-1 bg-[#111] border-white/10">
                    <button onClick={() => { setEditandoMensaje(msg.id); setEditInput(msg.contenido || ""); setMenuMensaje(null); }} className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-gray-300 hover:bg-white/5 rounded-md">
                      <Edit3 className="w-3 h-3" /> Editar
                    </button>
                    <button onClick={() => handleFijar(msg.id, !(msg as ChatMensaje).fijado)} className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-gray-300 hover:bg-white/5 rounded-md">
                      {(msg as ChatMensaje).fijado ? <PinOff className="w-3 h-3" /> : <Pin className="w-3 h-3" />}
                      {(msg as ChatMensaje).fijado ? "Desfijar" : "Fijar"}
                    </button>
                    <button onClick={() => handleEliminar(msg.id)} className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-red-400 hover:bg-white/5 rounded-md">
    ...
    <div className="w-[calc(100vw-32px)] max-w-[380px] h-[500px] sm:h-[600px] bg-[#0a0a0a] border border-white/10 rounded-[24px] sm:rounded-[32px] shadow-2xl shadow-black/50 flex flex-col overflow-hidden backdrop-blur-xl">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-[#0a0a0a]/90">
        <div className="flex items-center gap-3">
          {(vista === "chat" || vista === "visitante") && (
            <button onClick={() => setVista("lista")} className="p-1.5 hover:bg-white/5 rounded-lg transition-colors">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-black text-white uppercase tracking-wider">
                {vista === "chat" && salaActiva ? salaActiva.nombre || "Chat" : vista === "visitante" ? "BLIS Chat" : "Mensajes"}
              </h3>
              {vista === "chat" && (
                <div className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                  <span className="text-[10px] text-gray-500 uppercase tracking-wider">En línea</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center gap-1">
          {vista === "chat" && (
            <>
              <button onClick={() => { setMostrarBusqueda(!mostrarBusqueda); if (mostrarBusqueda) { setQueryBusqueda(""); setMensajesBuscados([]); } }} className={`p-2 hover:bg-white/5 rounded-lg transition-colors ${mostrarBusqueda ? "text-white bg-white/10" : "text-gray-400 hover:text-white"}`}>
                <Search className="w-4 h-4" />
              </button>
              {remoteUserId && (
                <>
                  <Tooltip><TooltipTrigger><button onClick={() => iniciarLlamada(remoteUserId, "audio")} className="p-2 hover:bg-white/5 rounded-lg transition-colors text-gray-400 hover:text-white"><Phone className="w-4 h-4" /></button></TooltipTrigger><TooltipContent>Llamada de voz</TooltipContent></Tooltip>
                  <Tooltip><TooltipTrigger><button onClick={() => iniciarLlamada(remoteUserId, "video")} className="p-2 hover:bg-white/5 rounded-lg transition-colors text-gray-400 hover:text-white"><Video className="w-4 h-4" /></button></TooltipTrigger><TooltipContent>Videollamada</TooltipContent></Tooltip>
                </>
              )}
            </>
          )}
          {user && pushPermission === "default" && (
            <button onClick={requestPermission} className="p-2 hover:bg-white/5 rounded-lg transition-colors text-amber-400 hover:text-amber-300" title="Activar notificaciones">
      ...
      <div className="flex-1 overflow-hidden relative">
        <AnimatePresence mode="wait">
          ...
          {vista === "lista" && (
            <motion.div key="lista" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-full flex flex-col">
              {!user ? (
                <div className="p-6 space-y-6 flex-1 flex flex-col justify-center">
                  <div className="text-center">
                    ...
                    <h4 className="text-lg font-black text-white uppercase tracking-wider mb-2">¡Hola!</h4>
                    <p className="text-sm text-gray-400">Bienvenido a BLIS Corp. ¿En qué podemos ayudarte hoy?</p>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 block">Tu nombre</label>
                      <Input value={visitanteNombre} onChange={(e) => setVisitanteNombre(e.target.value)} placeholder="Ej: Juan Pérez" className="bg-white/5 border-white/10 text-white placeholder:text-gray-600" />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 block">Email (opcional)</label>
                      <Input type="email" value={visitanteEmail} onChange={(e) => setVisitanteEmail(e.target.value)} placeholder="juan@ejemplo.com" className="bg-white/5 border-white/10 text-white placeholder:text-gray-600" />
                    </div>
                    <Button onClick={iniciarChatVisitante} disabled={!visitanteNombre.trim()} className="w-full bg-blis-red hover:bg-blis-red/90 text-white font-black uppercase tracking-widest text-xs py-6">
                      Iniciar chat
                    </Button>
                  </div>
                  <p className="text-center text-[10px] text-gray-600">Al continuar, aceptas nuestra política de privacidad.</p>
                </div>
              ) : (
                <>
                  <div className="flex border-b border-white/5">
                    <button onClick={() => setTabMiembro("chats")} className={`flex-1 py-3 text-xs font-black uppercase tracking-wider transition-colors ${tabMiembro === "chats" ? "text-blis-red border-b-2 border-blis-red" : "text-gray-500 hover:text-gray-300"}`}>Chats</button>
                    <button onClick={() => setTabMiembro("contactos")} className={`flex-1 py-3 text-xs font-black uppercase tracking-wider transition-colors ${tabMiembro === "contactos" ? "text-blis-red border-b-2 border-blis-red" : "text-gray-500 hover:text-gray-300"}`}>Contactos</button>
                  </div>

                  {tabMiembro === "chats" ? (
                    salas.length === 0 ? (
                      <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
                        <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center mb-4"><MessageCircle className="w-8 h-8 text-gray-600" /></div>
                        <h4 className="text-sm font-black text-white uppercase tracking-wider mb-2">Sin conversaciones</h4>
                        <p className="text-xs text-gray-500">Aún no tienes chats activos. Ve a Contactos para iniciar una conversación.</p>
                      </div>
                    ) : (
                            ...
                            <button key={sala.id} onClick={async () => { await unirseSala(sala.id); setVista("chat"); }} className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-white/5 transition-colors text-left group">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className="text-[10px] text-gray-600">{sala.ultima_actividad ? new Date(sala.ultima_actividad).toLocaleDateString("es-ES", { day: "numeric", month: "short" }) : ""}</span>
                                </div>
                                <p className="text-xs text-gray-500 truncate group-hover:text-gray-400 transition-colors">{sala.ultimo_mensaje?.contenido || "Sin mensajes"}</p>
                              </div>
                            </button>
                          ))}
                        </div>
                      ...
                      <div className="p-3 space-y-1">
                        {cargandoContactos ? (
                          ...
                          contactos.map((contacto) => (
                            <button key={contacto.id} onClick={() => handleNuevoChatContacto(contacto)} className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-white/5 transition-colors text-left group">
                              <div className="relative">
                                <Avatar className="w-10 h-10"><AvatarImage src={contacto.avatar_url || undefined} /><AvatarFallback className="bg-blis-red/20 text-blis-red text-xs font-black">{contacto.nombre?.[0] || "U"}</AvatarFallback></Avatar>
                                <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#0a0a0a] ${contacto.estado_chat === "online" ? "bg-emerald-500" : contacto.estado_chat === "ausente" ? "bg-amber-500" : "bg-gray-600"}`} />
                              </div>
                              <div className="flex-1 min-w-0">
                                <h5 className="text-sm font-bold text-white truncate">{contacto.nombre}</h5>
              ...
              {mensajes.some((m) => m.fijado) && (
                <div className="px-4 py-2 border-b border-white/5 bg-white/[0.02]">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Fijado</span>
                  </div>
                ...
                <div className="px-4 py-4">
                  {mensajes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center">
                      ...
                      <p className="text-sm text-gray-500">{salaActiva.tipo === "ia" ? "Hola, soy tu asistente virtual. ¿En qué puedo ayudarte?" : "Inicia la conversación..."}</p>
                    </div>
                  ) : (
                    <>
              ...
              <div className="px-4 py-3 border-t border-white/5 bg-[#0a0a0a]/90">
                <div className="flex items-end gap-2">
                  <div className="flex-1 relative">
                    <Input value={mensajeInput} onChange={(e) => handleInputChange(e.target.value)} onKeyDown={handleKeyDown} placeholder="Escribe un mensaje..." className="bg-white/5 border-white/10 text-white placeholder:text-gray-600 pr-10 py-6 rounded-2xl" disabled={enviando} />
                  </div>
                  <div className="flex items-center gap-1">
                    <Popover>
                    ...
                    <Tooltip><TooltipTrigger><button onClick={() => fileInputRef.current?.click()} className="p-2.5 hover:bg-white/5 rounded-xl transition-colors text-gray-400 hover:text-white"><Paperclip className="w-5 h-5" /></button></TooltipTrigger><TooltipContent>Adjuntar archivo</TooltipContent></Tooltip>

                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleEnviar} disabled={!mensajeInput.trim() || enviando} className="p-4 bg-blis-red rounded-xl text-white hover:bg-blis-red/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed min-h-[48px] min-w-[48px] flex items-center justify-center">
            ...
            <motion.div key="visitante" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="h-full flex flex-col">
              ...
              <div className="px-4 py-3 border-t border-white/5">
                <div className="flex items-end gap-2">
                  ...
                  >
                    {visitanteEnviando ? <Loader2 className="w-6 h-6 animate-spin" /> : <Send className="w-6 h-6" />}
                  </motion.button>
                </div>
              </div>
            </motion.div>
