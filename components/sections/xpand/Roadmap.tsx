    ...
    <section className="relative bg-[#0a0a0a] py-20 md:py-28 overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-4 md:px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12 md:mb-16">
          <span className="text-[#d5c108] text-xs md:text-sm font-semibold tracking-widest uppercase">Tu Camino</span>
          <h2 className="text-2xl md:text-5xl font-bold mt-2 md:mt-3 mb-3 md:mb-4">Roadmap del Trader</h2>
          <p className="text-white/50 text-sm md:text-base max-w-xl mx-auto">Cinco etapas que te llevarán de principiante a trader consistente.</p>
        </motion.div>

        <div className="relative">
          <div className="absolute left-[19px] md:left-1/2 top-0 bottom-0 w-px bg-white/5 md:-translate-x-px" />
          ...
          {stages.map((s, i) => (
            ...
            >
              <div className="relative z-10 flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#0a0a0a] border-2 border-[#d5c108]/50 flex items-center justify-center">
                <s.icon className="w-4 h-4 md:w-5 md:h-5 text-[#d5c108]" />
              </div>

              <div className={`flex-1 md:w-1/2 ${i % 2 === 0 ? "md:text-right md:pr-12" : "md:text-left md:pl-12"}`}>
                <div className="bg-white/[0.03] border border-white/5 rounded-xl p-4 md:p-6 hover:border-[#d5c108]/30 transition-all">
                  <span className="text-[#d5c108] text-xs font-mono">Etapa {i + 1}</span>
                  <h3 className="text-lg md:text-xl font-bold mt-1 mb-2">{s.title}</h3>
