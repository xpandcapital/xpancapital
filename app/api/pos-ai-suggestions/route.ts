...
export async function POST(request: NextRequest) {
    try {
        const { cart, context, catalog } = await request.json()
        ...
        if (!activeKey && !activeGptKey && !activeGroqKey) {
            return NextResponse.json({ error: "No se configuraron llaves de IA (Gemini, GPT ni Groq). Agrégalas en API Nube." }, { status: 400 })
        }
