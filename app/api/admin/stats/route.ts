...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      supabase.from('profiles').select('id, nombre, apellido, blis_coins, creado_en').eq('empresa_id', empresa_id).order('blis_coins', { ascending: false }).limit(5),
    ])
    ...
    const [{ data: coinsData }, { data: blogViews }, { data: purchases }] = await Promise.all([
      supabase.from('profiles').select('blis_coins').eq('empresa_id', empresa_id),
    ...
    const totalCoins = (coinsData || []).reduce((sum, p) => sum + (p.blis_coins || 0), 0)
