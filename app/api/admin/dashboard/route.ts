...
export async function GET() {
  try {
    ...
    const [
      ...
      supabase.from("compras").select("id, monto_usd, estado, creado_en, user_id, metadata, cliente:profiles!user_id(id, nombre, apellido)").eq("estado", "completado").order("creado_en", { ascending: false }).limit(5),
