...
import { 
  ...
  PauseCircle, PlayCircle, LayoutDashboard, Users,
...
const App = () => {
  ...
  const [projectList, setProjectList] = useState(() => {
    try {
      ...
      console.warn("Error leyendo project_list", e);
    }
    return [{ id: 'proj_default', name: 'Mi Primer Proyecto' }];
  });

  const [activeProjectId, setActiveProjectId] = useState(() => {
    try {
      return localStorage.getItem('inmo_active_project') || projectList[0]?.id || 'proj_default';
    } catch (e) {
      return projectList[0]?.id || 'proj_default';
    }
  });
    ...
    try {
      ...
      if (savedLots) {
        ...
        parsedLots = parsedLots.map(l => {
          ...
          // MIGRACIÓN: Eliminar signaturePaymentExpected y pasarlo a initialPayments si existe
      ...
      const savedConfigStr = localStorage.getItem(`inmo_proj_config_meta_${activeProjectId}`);
      if (savedConfigStr) {
          try {
             const parsedMeta = JSON.parse(savedConfigStr);
             setProjectConfig(prev => ({
                 ...prev, 
                 startMonth: parsedMeta.startMonth || '2025-04', 
                 signatureMonth: parsedMeta.signatureMonth || '2026-04', 
                 escrituraMonth: parsedMeta.escrituraMonth || '2027-01',
                 lotPins: parsedMeta.lotPins || []
             }));
          } catch(e) {}
      } else {
          setProjectConfig({ startMonth: '2025-04', signatureMonth: '2026-04', escrituraMonth: '2027-01', masterplanImage: null, lotPins: [] });
      }
      
      const savedImage = localStorage.getItem(`inmo_proj_config_img_${activeProjectId}`);
      if (savedImage) {
          setProjectConfig(prev => ({...prev, masterplanImage: savedImage}));
      } else {
          setProjectConfig(prev => ({...prev, masterplanImage: null}));
      }
  ...
  // Sync lots to Supabase
  const syncLotsToSupabase = useCallback(async (lotsData) => {
    if (!activeProjectId || !lotsData || lotsData.length === 0) return;
    try {
      for (const lot of lotsData) {
        const supabaseLot = {
          ...
          start_month: lot.startMonth || '2025-04',
          signature_month: lot.signatureMonth || '2026-04',
          escritura_month: lot.escrituraMonth || '2027-01',
        ...
        if (lot.id && lot.id.startsWith('lot_')) {
          await supabase.from('project_lots').upsert({
            id: lot.id,
            ...supabaseLot,
            updated_at: new Date().toISOString()
          });
        } else {
          const { data } = await supabase.from('project_lots').insert(supabaseLot).select();
          if (data && data[0]) {
            lot.id = data[0].id;
          }
        }
      }
      console.log('[GestionDeLotes] Synced' + lotsData.length + ' lots to Supabase');
    } catch (err) {
      console.error('[GestionDeLotes] Sync error:', err);
    }
  }, [activeProjectId]);
  ...
  const generateAIPromptText = (validMonths) => {
    ...
    DEBES CLASIFICAR la información en CONTRATOS (documentos legales que definen dueños, áreas y precios) y RECIBOS (comprobantes de pago o transferencias).
    Devuelve un JSON estricto con esta estructura:
    {
      "contratosEncontrados": [
        {
          "loteNumber": "Identificador del lote (Ej: Lote 01, Lote 02)",
          "lotArea": "Área o medidas del lote en m2 (solo número, 0 si no hay)",
          "agentName": "Nombre del asesor o vendedor comercial (si aparece)",
          "owners": [
             {"name": "Nombre completo", "documentId": "Cédula o Pasaporte si aparece", "email": "Correo si aparece", "phone": "Celular si aparece"}
          ],
          "alternateContact": {
             "name": "Si el contrato menciona a una 'persona designada en caso de ausencia' o apoderado, pon su nombre aquí.",
             "phone": "Teléfono del contacto alterno si lo hay"
          },
          "startMonthOverride": "Si el contrato indica explicitamente en qué mes empiezan las 'cuotas mensuales' (ej. 'agosto de 2025'), devuelve el mes en formato YYYY-MM (ej. '2025-08'). Si no lo dice claro, devuelve null",
          "totalPrice": Precio total de venta (solo número, 0 si no hay),
          "expectedQuota": Cuota mensual pactada regular predominante (solo número, 0 si no hay),
          "tradeInValue": "Valor de descuentos, bonos, o canjes (Ej. si dice 'la compradora autoriza que sus comisiones sean abonadas al lote', y especifica un monto, ponlo aquí. 0 si no hay).",
          "specialObservations": "Extrae notas importantes o inusuales como: 'Sorteo de vehículo', 'Pago con comisiones de asesor', 'Penalidades especiales', 'Letras de cambio', renegociaciones, etc.",
    ...
    Importante: 
    1. Si hay VARIOS compradores, añádelos a TODOS en el array de 'owners'.
    ...
    3. Extrae SOLO el monto pagado para los recibos e ignora los saldos restantes de las imágenes.
    4. Si en un documento imagen hay recibos de VARIOS lotes, crea un objeto en "recibosEncontrados" por cada pago y asigna correctamente a qué "loteNumber" pertenece.
    5. Es VITAL que en "archivoOrigen" copies exactamente el nombre del documento de donde extrajiste la información.`;
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (aiData.contratosEncontrados && aiData.contratosEncontrados.length > 0) {
          ...
          if (extracted.agentName) updatedLot.agentName = extracted.agentName;
        ...
        if (aiData.recibosEncontrados && Array.isArray(aiData.recibosEncontrados)) {
          setProcessingLog(prev => [...prev, `Se encontraron ${aiData.recibosEncontrados.length} recibos. Asignándolos a cuotas...`]);
        ...
        setProcessingLog(prev => [...prev, `¡Proceso finalizado exitosamente!`]);
        setTimeout(() => showAlert('Éxito', 'Documentos procesados e integrados correctamente al expediente.'), 500);
      }
    } catch (error) {
      console.error(error);
      setProcessingLog(prev => [...prev, `❌ Error: Hubo un problema al comunicarse con la IA.`]);
  ...
  const handleMassiveUpload = async (e) => {
    ...
    setProcessingLog([`Iniciando análisis masivo de ${files.length} documentos...`]);
    ...
    const apiKey = "AIzaSyDTaDqoOzRBeDlZlS2rvUFse9aLMVHUsHU"; 
    const CHUNK_SIZE = 3; 
    let currentLotsMap = new window.Map();
    
    lots.forEach(l => currentLotsMap.set(l.loteNumber, l));
    ...
    for (let i = 0; i < files.length; i += CHUNK_SIZE) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (aiData.contratosEncontrados && Array.isArray(aiData.contratosEncontrados)) {
            aiData.contratosEncontrados.forEach(extracted => {
              ...
              let existingLot = currentLotsMap.get(lId);
              let effectiveStartMonth = extracted.startMonthOverride || projectConfig.startMonth;
              
              if (!existingLot) {
                setProcessingLog(prev => [...prev, `✨ Creando nuevo expediente para el Lote ${lId}...`]);
                existingLot = {
                  id: crypto.randomUUID(),
                  loteNumber: lId,
                  documents: [],
                ...
                setProcessingLog(prev => [...prev, `🔄 Actualizando información del contrato existente para el Lote ${lId}...`]);
                if (extracted.owners && extracted.owners.length > 0) {
                  existingLot.owners = extracted.owners.map(o => ({ id: crypto.randomUUID(), name: o.name || '', documentId: o.documentId || '', email: o.email || '', phoneCode: '+593', phone: o.phone || '' }));
                  existingLot.clientName = existingLot.owners.map(o => o.name).join(' y ');
                }
                ...
                if(extracted.agentName) existingLot.agentName = extracted.agentName;
              ...
              currentLotsMap.set(lId, existingLot);
            });
          }
        ...
        setProcessingLog(prev => [...prev, `❌ Error crítico al analizar el paquete de documentos.`]);
      }
    }

    setProcessingLog(prev => [...prev, `💳 Clasificando y enlazando ${pendingReceipts.length} recibos con sus respectivos contratos...`]);
    ...
    pendingReceipts.forEach(recibo => {
      ...
      if (existingLot) {
        ...
        setProcessingLog(prev => [...prev, `⚠️ Recibo detectado para Lote ${lId} ignorado porque no existe su contrato base.`]);
      }
    });

    setLots(Array.from(currentLotsMap.values()));
    setProcessingLog(prev => [...prev, `¡Finalizado con éxito! Redirigiendo al Dashboard...`]);
    ...
    setTimeout(() => {
      ...
      if (missingContractLots.size > 0) {
        ...
        showAlert(
          '⚠️ Contratos Faltantes', 
          `La IA encontró recibos para: ${lotesFaltantes}. \nSin embargo, NO se ha subido el Contrato principal de estos lotes. Súbelos para asociarlos correctamente.`
        );
      }
    }, 1500);
  };

  // --- 3. CÁLCULOS GLOBALES Y ORDENAMIENTO ---
  ...
  const toggleLotStatus = (lot) => {
    if (lot.status === 'Activo') {
      showPrompt('Desistir Lote', '¿Cuánto dinero se le va a devolver al cliente por este desistimiento? (Pon 0 si la penalidad es del 100% o si no pagó nada).', '0', (val) => {
        updateSelectedLot({ status: 'Desistido', refundAmount: Number(val) || 0 });
      });
    } else {
      updateSelectedLot({ status: 'Activo', refundAmount: 0 });
    }
  };
        ...
        <body>
          <div class="cert-container">
            ...
            <div class="audit-info">
              <p style="text-align: center; font-weight: bold; margin-top: 0; margin-bottom: 15px; border-bottom: 1px solid #e5e7eb; padding-bottom: 10px;">DATOS DE AUDITORÍA CRIPTOGRÁFICA</p>
  ...
  // --- 6. COMUNICACIÓN (WHATSAPP, CORREO, PDF) Y MENSAJES ---
  const generateMessageForClient = async () => {
    ...
    if (lot.status === 'Desistido') {
        roleInstruction = "Actúa como el Departamento Legal del proyecto.";
        toneInstructions = `El cliente ha desistido del contrato. Redacta un documento formal indicando la resolución del contrato de reserva. Menciona que se ha procedido a la liberación del lote y que se le devolverá un monto de $${lot.refundAmount || 0} por concepto de liquidación final, dando por terminada la relación comercial.`;
    } else if (messageTone === 'Asesor Comercial') {
        roleInstruction = "Actúa como el Asesor Comercial personal del cliente.";
        toneInstructions = "Tono muy cercano, entusiasta y servicial. Enfócate en el sueño de tener su nuevo hogar. Minimiza la fricción llamando a la deuda 'saldos pendientes para poder avanzar con tu firma'. Usa lenguaje optimista.";
    } else if (messageTone === 'Departamento Legal') {
        roleInstruction = "Actúa como el Departamento Legal de la constructora/inmobiliaria.";
        toneInstructions = "Tono estricto, muy formal y riguroso. Usa términos como 'obligaciones contractuales', 'deuda', 'vencimiento', 'requerimiento' y 'regularización inmediata'. Menciona que el pago es requisito indispensable para no incurrir en incumplimientos.";
    } else if (messageTone === 'Gerente General') {
        roleInstruction = "Actúa como el Gerente General del proyecto inmobiliario.";
        toneInstructions = "Tono institucional, de alto nivel, agradecido pero asertivo y firme. Habla del compromiso mutuo, el gran avance del proyecto y la importancia de su aporte financiero para el éxito colectivo de la urbanización.";
    } else {
        roleInstruction = "Actúa como el Departamento de Administración del proyecto.";
        toneInstructions = "Tono amigable, formal y colaborativo. Si tuvo retención autorizada, pide la 'liberación de los fondos retenidos' para continuar el desarrollo. Si no, pide la integración de cuotas pendientes de forma cordial, sin usar la palabra deuda excesivamente.";
    }
    ...
    const promptText = `
      ...
      Datos financieros exactos de este cliente (Si está desistido, ignora los pagos futuros):
      ...
      - ¿Tuvo retención de cuotas autorizada previamente por demoras?: ${lot.conditions.authorizedHold ? 'Sí' : 'No'}

      Instrucciones de Redacción:
      1. ${toneInstructions}
      2. Dirígete a todos los propietarios mencionados de forma plural si son varios.
      ...
      4. ${lot.status === 'Activo' ? `Menciona que su saldo proyectado para la escritura definitiva en ${lotEscrituraName} será de $${saldoEscritura} (tras el pago de cuotas futuras).` : ''}
      5. Máximo 3 párrafos.
    `;
  ...
  const handleSendEmail = (lot) => {
    ...
    const url = `mailto:${lot.owners[0].email}?subject=Notificación Inmobiliaria - ${lot.loteNumber}&body=${encodeURIComponent(lot.generatedMessage)}`;
    window.open(url, '_blank');
  };
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <p>En la ciudad de residencia, a la fecha de suscripción, comparecen para celebrar de manera libre y voluntaria el presente Contrato de Reserva de Lote de Terreno, las siguientes partes:</p>
          ...
          <p>El(Los) comprador(es) reserva(n) para sí el lote <strong>${lot.loteNumber}</strong>, con un área de <strong>${lot.lotArea} m2</strong>, dentro del proyecto mencionado.</p>
          
          <h2>CLÁUSULA SEGUNDA: PRECIO Y FORMA DE PAGO</h2>
          <p>Las partes acuerdan que el precio total del lote es de <strong>${formatCurrency(lot.totalPrice)}</strong>, el cual será pagado de la siguiente manera:</p>
          <ul>
            <li><strong>Cuota Mensual Acordada:</strong> ${formatCurrency(lot.expectedQuota)}</li>
          </ul>
          
          <h2>CLÁUSULA TERCERA: CONDICIONES ESPECIALES</h2>
    ...
    <div className="max-w-7xl mx-auto mt-6 space-y-6 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
          ...
          Estado de Cobranzas: {activeProjectName}
        </h2>
        <div className="flex flex-wrap gap-2">
          <button onClick={exportToCSV} className="bg-zinc-800 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-zinc-700 border border-zinc-700 transition-colors flex items-center gap-2">
            <FileSpreadsheet className="w-4 h-4 text-emerald-400" /> Excel
          </button>
          <button onClick={exportToJSON} className="bg-zinc-800 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-zinc-700 border border-zinc-700 transition-colors flex items-center gap-2">
            <Download className="w-4 h-4 text-zinc-400" /> Guardar JSON
          </button>
          <button onClick={() => setView('upload')} className="bg-rose-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-rose-700 transition-colors">
            <Settings className="w-4 h-4 inline mr-1" /> Configurar / Subir Docs
          </button>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-zinc-900 p-4 rounded-xl shadow-sm border border-zinc-800 border-l-4 border-l-zinc-500">
          <p className="text-[11px] text-zinc-400 font-bold uppercase tracking-wider">Lotes Activos</p>
          <div className="flex items-baseline gap-2 mt-1">
            <p className="text-2xl font-bold text-white">{dashboardStats.activeLotsCount}</p>
            {lots.length - dashboardStats.activeLotsCount > 0 && <p className="text-xs text-rose-500 font-bold">({lots.length - dashboardStats.activeLotsCount} desistidos)</p>}
          </div>
        </div>
        <div className="bg-zinc-900 p-4 rounded-xl shadow-sm border border-zinc-800 border-l-4 border-l-emerald-500">
          <p className="text-[11px] text-zinc-400 font-bold uppercase tracking-wider">Recaudado (Empresa)</p>
          <p className="text-xl font-bold text-emerald-400 mt-1">{formatCurrency(dashboardStats.totalCollectedSoFar)}</p>
        </div>
        <div className="bg-zinc-900 p-4 rounded-xl shadow-sm border border-zinc-800 border-l-4 border-l-rose-500">
          <p className="text-[11px] text-zinc-400 font-bold uppercase tracking-wider">A Cobrar Promesa</p>
          <p className="text-xl font-bold text-rose-500 mt-1">{formatCurrency(dashboardStats.totalToCollectNow)}</p>
        </div>
        <div className="bg-zinc-900 p-4 rounded-xl shadow-sm border border-zinc-800 border-l-4 border-l-cyan-500">
          <p className="text-[11px] text-zinc-400 font-bold uppercase tracking-wider">Cuotas Futuras</p>
          <p className="text-xl font-bold text-cyan-400 mt-1">{formatCurrency(dashboardStats.totalFutureQuotas)}</p>
        </div>
        <div className="bg-black p-4 rounded-xl shadow-sm border border-zinc-800 border-l-4 border-l-amber-500">
          <p className="text-[11px] text-zinc-400 font-bold uppercase tracking-wider">Saldo a Escrituras</p>
          <p className="text-xl font-bold text-amber-500 mt-1">{formatCurrency(dashboardStats.totalSaldoEscritura)}</p>
        </div>
      </div>

      <div className="bg-zinc-900 rounded-xl shadow-sm border border-zinc-800 overflow-hidden">
        <div className="p-4 border-b border-zinc-800 bg-zinc-950/50">
          <h3 className="font-semibold text-zinc-300">Resumen Detallado por Lote</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-zinc-950/50 text-zinc-500 text-[10px] uppercase tracking-wider border-b border-zinc-800">
                <th className="p-4 font-medium">Lote / Cliente</th>
                <th className="p-4 font-medium text-center">Docs</th>
                <th className="p-4 font-medium">Pagado Histórico</th>
                <th className="p-4 font-medium text-rose-500">A Cobrar (Promesa)</th>
                <th className="p-4 font-medium text-cyan-500">Cuotas Futuras</th>
                <th className="p-4 font-medium text-amber-500">Saldo Escritura</th>
            ...
            <tbody className="divide-y divide-zinc-800">
              {sortedLots.length === 0 && (
                <tr>
                  <td colSpan="7" className="p-16 text-center bg-zinc-900">
                    <FolderOpen className="w-16 h-16 text-zinc-700 mx-auto mb-4" />
                    <p className="text-zinc-400 font-semibold text-lg">Aún no hay lotes procesados</p>
                    <p className="text-sm text-zinc-600 mt-2">Haz clic en el botón "Configurar / Subir Docs" en la parte superior para comenzar.</p>
                  </td>
                </tr>
              )}
              {sortedLots.map(lot => {
                ...
                return (
                  <tr key={lot.id} className={`hover:bg-zinc-800/50 transition-colors ${isDesistido ? 'opacity-50' : ''}`}>
                    <td className="p-4">
                      <div className="font-bold text-white flex items-center gap-2">
                        {lot.loteNumber} 
                        {isDesistido && <span className="bg-rose-950 text-rose-500 border border-rose-900 text-[9px] px-1.5 py-0.5 rounded-sm uppercase">Liberado</span>}
                        {lot.entersRaffle && <Ticket className="w-3 h-3 text-amber-500" title="Participa en Sorteo"/>}
                      </div>
                      <div className={`font-medium text-sm truncate max-w-[200px] ${isDesistido ? 'text-zinc-600 line-through' : 'text-zinc-300'}`} title={lot.clientName}>{lot.clientName}</div>
                      <div className="text-[10px] text-zinc-500 mt-0.5">P. Venta: {formatCurrency(lot.totalPrice)}</div>
                    </td>
                    <td className="p-4 text-xs text-zinc-400 text-center">
                      <span className="bg-zinc-800 border border-zinc-700 px-2 py-1 rounded-md">{lot.documents?.length || 0}</span>
                    </td>
                    <td className="p-4 text-zinc-300 font-medium text-sm">
                      {isDesistido ? (
                        <div>
                          <span className="line-through text-zinc-600">{formatCurrency(paid)}</span>
                          <div className="text-[9px] text-rose-500 font-bold mt-0.5">- {formatCurrency(lot.refundAmount || 0)} Devuelto</div>
                        </div>
                      ) : formatCurrency(paid)}
                    </td>
                    <td className="p-4 font-bold text-sm">
                      {isDesistido ? <span className="text-zinc-600">-</span> : (toPayNow > 0 ? <span className="text-rose-500">{formatCurrency(toPayNow)} <br/><span className="text-[9px] font-normal text-zinc-500">En {formatMonthYear(sigMonth)}</span></span> : <span className="text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded-md text-xs"><CheckCircle2 className="w-3 h-3 inline mr-1"/> Al día</span>)}
                    </td>
                    <td className="p-4 font-medium text-cyan-500 text-sm">
                      {isDesistido ? <span className="text-zinc-600">-</span> : <>{formatCurrency(futureQuotasTotal)} <span className="text-[9px] font-normal text-cyan-800">({lotFutureMonths}m)</span></>}
                    </td>
                    <td className="p-4 font-bold text-amber-500 text-sm">
                      {isDesistido ? <span className="text-zinc-600">-</span> : <>{formatCurrency(saldoEscritura)} <br/><span className="text-[9px] font-normal text-amber-700/50">En {formatMonthYear(escMonth)}</span></>}
                    </td>
                    <td className="p-4 text-right">
                      <button 
                        onClick={() => { setSelectedLotId(lot.id); setView('detail'); }}
                        className="text-white border border-zinc-600 px-3 py-1.5 rounded-md hover:bg-zinc-800 hover:border-zinc-400 transition-colors text-xs font-medium uppercase tracking-wide mr-2"
                      >
                        Auditar
                      </button>
                      <button 
                        onClick={() => deleteLot(lot.id)}
                        className="text-rose-500/70 hover:text-rose-500 transition-colors p-1.5 rounded hover:bg-rose-950/30 inline-flex align-middle"
  ...
  const renderDetailView = () => {
    ...
    const lotStartMonth = lot.startMonth || projectConfig.startMonth;
    const sigMonth = lot.signatureMonth || projectConfig.signatureMonth;
    const escMonth = lot.escrituraMonth || projectConfig.escrituraMonth;
    const lotFutureMonths = Math.max(0, getMonthsDifference(sigMonth, escMonth));
    const lotSignatureName = formatMonthYear(sigMonth);
    ...
    return (
      <div className="max-w-6xl mx-auto mt-6 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl overflow-hidden animate-fadeIn pb-8">
        
        {/* ESTADO DESISTIDO BANNER */}
        {isDesistido && (
          <div className="bg-rose-900/80 text-white p-3 text-center font-bold flex items-center justify-center gap-2 border-b border-rose-950">
            <ShieldAlert className="w-5 h-5"/> LOTE LIBERADO - EL CLIENTE DESISTIÓ DE LA COMPRA
          </div>
        )}

        <div className="bg-black/50 text-white p-4 flex items-center justify-between border-b border-zinc-800">
          <div className="flex items-center gap-4">
            <button onClick={() => setView('dashboard')} className="p-2 hover:bg-zinc-800 rounded-lg transition-colors text-zinc-400 hover:text-white">
          ...
          <div className="text-right hidden sm:flex items-center gap-4">
             {!isDesistido && (
                <button onClick={() => toggleLotStatus(lot)} className="bg-rose-950/50 hover:bg-rose-900 border border-rose-900 text-rose-300 px-3 py-1.5 rounded text-xs font-bold transition-colors">
                  Desistir / Liberar Lote
                </button>
             )}
             {isDesistido && (
                <button onClick={() => toggleLotStatus(lot)} className="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1.5 rounded text-xs font-bold transition-colors flex items-center gap-1 border border-zinc-700">
             ...
             {!isDesistido && (
               <div>
                 ...
                 <div className={`text-2xl font-bold ${totalToPayNow > 0 ? 'text-rose-500' : 'text-emerald-400'}`}>
                    {totalToPayNow > 0 ? formatCurrency(totalToPayNow) : 'AL DÍA'}
                 </div>
               </div>
             )}
          </div>
        ...
        <div className={`p-6 space-y-6 ${isDesistido ? 'opacity-70' : ''}`}>
              ...
              <div className="bg-zinc-950/50 p-5 rounded-xl border border-zinc-800 shadow-sm">
                ...
                <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                           <div className="flex-1">
                              <label className="text-[9px] font-bold text-zinc-500 uppercase block mb-0.5">Teléfono</label>
                  ...
                  <button onClick={addOwner} className="flex items-center gap-1 text-xs font-bold text-rose-400 hover:text-rose-300 transition-colors w-full justify-center border border-dashed border-zinc-700 rounded p-2 hover:bg-zinc-800">
                    <PlusCircle className="w-4 h-4" /> Añadir otro titular
                  </button>
                  ...
                  <div className="pt-3 border-t border-zinc-800 mt-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex bg-zinc-900 border border-zinc-800 rounded overflow-hidden focus-within:border-rose-500">
                        ...
                        <input type="tel" value={lot.alternateContact?.phone || ''} onChange={(e) => updateSelectedLot({ alternateContact: { ...lot.alternateContact, phone: e.target.value } })} className="w-full p-1.5 text-xs bg-transparent outline-none text-white placeholder-zinc-600" placeholder="Teléfono (Opcional)" />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-zinc-950/50 p-5 rounded-xl border border-zinc-800 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    ...
                    <div className="w-1/3">
                      <label className="text-[11px] font-bold text-zinc-500 uppercase">Área ($m^2$)</label>
                  ...
                  {lot.agentName && (
                    <div className="mt-3 p-3 bg-zinc-900 rounded border border-emerald-900/30">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        {isCommissionReady ? (
                          <span className="text-[10px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded uppercase font-bold">✓ Liberada</span>
                        ) : (
                          <span className="text-[10px] bg-amber-500/20 text-amber-500 border border-amber-500/30 px-2 py-0.5 rounded uppercase font-bold">⏳ Pendiente</span>
                        )}
                      </div>
                    </div>
                  )}
            ...
            <div className="xl:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800 flex flex-col justify-between shadow-sm">
                    <div className="mb-3">
                      <h4 className="font-semibold text-white flex items-center gap-2 mb-1"><MessageSquare className="w-4 h-4 text-rose-500" /> Bitácora y Observaciones Legales</h4>
                    </div>
                    <textarea 
                    ...
                    <div className="mt-4 pt-3 border-t border-zinc-800 flex items-center justify-between">
                      <span className="text-xs font-bold text-zinc-400">🎫 Participa en Sorteo Especial</span>
                      <button 
              ...
              {/* SECCIÓN 1: ABONOS INICIALES / RESERVA */}
              <div className="bg-zinc-950/50 p-5 rounded-xl border border-zinc-800 shadow-sm">
                    ...
                    <div key={ip.id} className={`flex flex-col md:flex-row items-center gap-3 p-3 rounded-lg border ${ip.expected > 0 && ip.actual < ip.expected ? 'bg-rose-950/10 border-rose-900/50' : 'bg-zinc-900 border-zinc-800'}`}>
                      ...
                      <div className="w-full md:w-1/4 flex flex-col gap-1">
                         ...
                         <label className="cursor-pointer text-[9px] font-bold bg-zinc-800 text-zinc-300 border border-zinc-700 px-2 py-1 rounded hover:bg-zinc-700 transition-colors text-center truncate" title={ip.receiptAttached}>
                           {ip.receiptAttached ? `✓ ${ip.receiptAttached}` : '📎 Subir Comp.'}
              ...
              {/* SECCIÓN 2: HISTÓRICO DE CUOTAS (COLAPSABLE) */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4 border-t border-zinc-800">
            ...
            <div className={`bg-black text-white rounded-2xl p-6 relative overflow-hidden shadow-lg border border-zinc-800 ${isDesistido ? 'opacity-80' : ''}`}>
              ...
              <h3 className="text-sm font-bold uppercase tracking-wider mb-4 text-zinc-400 relative z-10">Matemática del Contrato</h3>
      ...
      <div className="max-w-6xl mx-auto mt-6 space-y-6 animate-fadeIn">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
          <div>
            ...
            <p className="text-zinc-400 mt-1">Realiza sorteos aleatorios criptográficamente seguros para tus clientes calificados.</p>
          </div>
          ...
          {projectConfig.masterplanImage && (
            <div className="w-full lg:w-64 bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col h-[500px]">
              ...
              <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto mb-4 scrollbar-thin">
                {unmappedLots.length === 0 ? <span className="text-emerald-400 font-bold text-xs mt-1">¡Todos mapeados!</span> : 
                  unmappedLots.map(l => (
              ...
              <h3 className="font-bold text-white mb-2 text-sm border-b border-zinc-800 pb-2 mt-2">📌 Pines Colocados</h3>
  ...
  return (
    <div className="min-h-screen font-sans text-zinc-100 relative bg-[#09090b]">
      ...
      <style>{`
        ...
        .scrollbar-thin::-webkit-scrollbar-track { background: #09090b; }
      ...
      <div className="bg-black border-b border-zinc-900 shadow-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="bg-rose-600/20 p-2 rounded-lg border border-rose-500/20">
              <Calculator className="w-6 h-6 text-rose-500" />
            </div>
            <div className="flex flex-col">
              <h1 className="text-xl font-bold text-white leading-tight uppercase tracking-tight">Gestión de lotes y clientes</h1>
              <div className="flex items-center mt-1">
                <select
                  ...
                  className="bg-transparent text-zinc-400 text-xs font-bold uppercase tracking-wider outline-none cursor-pointer hover:text-white transition-colors max-w-[200px] truncate"
                >
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar w-full md:w-auto">
            <button onClick={() => setView('dashboard')} className={`px-4 py-2 rounded-lg text-sm font-bold uppercase tracking-wide whitespace-nowrap transition-colors ${view === 'dashboard' ? 'bg-rose-600 text-white' : 'bg-transparent text-zinc-400 hover:bg-zinc-900 hover:text-white'}`}>Dashboard</button>
            <button onClick={() => setView('calendar')} className={`px-4 py-2 rounded-lg text-sm font-bold uppercase tracking-wide whitespace-nowrap transition-colors flex items-center gap-1 ${view === 'calendar' ? 'bg-rose-600 text-white' : 'bg-transparent text-zinc-400 hover:bg-zinc-900 hover:text-white'}`}><CalendarIcon className="w-4 h-4"/> Calendario</button>
            <button onClick={() => setView('masterplan')} className={`px-4 py-2 rounded-lg text-sm font-bold uppercase tracking-wide whitespace-nowrap transition-colors flex items-center gap-1 ${view === 'masterplan' ? 'bg-rose-600 text-white' : 'bg-transparent text-zinc-400 hover:bg-zinc-900 hover:text-white'}`}><MapIcon className="w-4 h-4"/> Masterplan</button>
            <button onClick={() => setView('raffle')} className={`px-4 py-2 rounded-lg text-sm font-bold uppercase tracking-wide whitespace-nowrap transition-colors flex items-center gap-1 ${view === 'raffle' ? 'bg-amber-500 text-amber-950' : 'bg-transparent text-amber-500 hover:bg-zinc-900 hover:text-amber-400'}`}><Trophy className="w-4 h-4"/> Sorteos</button>
            <button onClick={() => setView('upload')} className={`px-4 py-2 rounded-lg text-sm font-bold uppercase tracking-wide whitespace-nowrap transition-colors ${view === 'upload' ? 'bg-rose-600 text-white' : 'bg-transparent text-zinc-400 hover:bg-zinc-900 hover:text-white'}`}>Configuración</button>
          </div>
        </div>
      </div>
      ...
      {modalConfig.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl p-6 w-full max-w-sm animate-fadeIn">
            <div className="flex justify-between items-center mb-4 border-b border-zinc-800 pb-2">
              <h3 className="text-lg font-bold text-white uppercase">{modalConfig.title}</h3>
              <button onClick={closeModal} className="text-zinc-500 hover:text-rose-500 transition-colors"><X className="w-5 h-5"/></button>
            </div>
            <p className="text-zinc-300 mb-6 text-sm leading-relaxed whitespace-pre-wrap">{modalConfig.message}</p>
            ...
            {modalConfig.type === 'prompt' && (
              <input 
                ...
                className="w-full mb-6 p-2 bg-zinc-950 border border-zinc-700 text-white rounded outline-none focus:border-rose-500 font-bold text-lg transition-colors"
              />
            )}

            <div className="flex justify-end gap-3">
              {(modalConfig.type === 'confirm' || modalConfig.type === 'prompt') && (
                <button onClick={closeModal} className="px-4 py-2 rounded-lg text-sm font-bold text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors">Cancelar</button>
              )}
              <button 
                onClick={() => {
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold text-white transition-colors shadow-md ${modalConfig.type === 'confirm' ? 'bg-rose-600 hover:bg-rose-700' : 'bg-rose-600 hover:bg-rose-700'}`}
              >
