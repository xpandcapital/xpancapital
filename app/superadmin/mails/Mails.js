...
export default function Mails() {
  ...
  return (
    <div id="blismail-cms-root" className={`flex flex-col h-screen font-sans overflow-hidden transition-colors duration-300 ${mail.theme === 'dark' ? 'dark bg-[#0a0a0a] text-gray-200' : 'bg-gray-100 text-gray-800'}`}>
      ...
      <TemplatesModal show={mail.showTemplatesModal} onClose={() => mail.setShowTemplatesModal(false)} savedTemplates={mail.savedTemplates} onLoadTemplate={mail.handleLoadTemplate} onDeleteTemplate={mail.handleDeleteTemplate} onRefreshTemplates={async () => { const t = await mail.getTemplates(); if (t) mail.setSavedTemplates(t); }} />
