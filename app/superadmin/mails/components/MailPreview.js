        ...
        {blocks.length === 0 ? (
          ...
          <div className={`shadow-2xl transition-all duration-300 relative bg-white dark:bg-[#111111]`} style={{ fontFamily: settings.fontFamily, width: previewMode === 'mobile' ? '375px' : `${settings.width}px`, minHeight: '600px', backgroundColor: settings.containerBg, borderRadius: previewMode === 'mobile' ? '30px' : '8px', border: previewMode === 'mobile' ? '12px solid #222222' : 'none', overflow: 'hidden' }}>
            {displayBlocks.map((block, index) => (
              <div key={block.id} className={`relative group border-2 transition-colors ${selectedBlockId === block.id ? 'border-[#e11d48] z-10' : 'border-transparent hover:border-[#e11d48]/40'}`} onClick={(e) => { e.stopPropagation(); setSelectedBlockId(block.id); setActiveTab('blocks'); }} style={{ marginBottom: index < displayBlocks.length - 1 ? `${Math.max(0, parseInt(settings.sectionGap) || 0)}px` : 0 }}>
                <div className={`absolute right-0 top-0 translate-x-full ml-1 flex-col space-y-1 bg-white dark:bg-[#161616] p-1 rounded-lg shadow-xl border border-gray-200 dark:border-[#333333] z-50 ${selectedBlockId === block.id ? 'flex' : 'hidden group-hover:flex'}`}>
                  <button onClick={(e) => moveBlock(block.id, 'up', e)} className="p-1 text-gray-500 hover:text-[#e11d48]" disabled={index === 0}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m18 15-6-6-6 6"/></svg>
                  </button>
                  <button onClick={(e) => moveBlock(block.id, 'down', e)} className="p-1 text-gray-500 hover:text-[#e11d48]" disabled={index === blocks.length - 1}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6"/></svg>
                  </button>
                  <button onClick={(e) => removeBlock(block.id, e)} className="p-1 text-red-500">
