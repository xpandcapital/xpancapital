...
import { X, Send, FolderOpen, Database, Code, Check, CheckCircle, Server, Star, Pencil, AlertCircle, Loader2, Paperclip, Trash2, Upload, Grid, Mail, Layers, Search, Plus, Users, Save, Copy } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#111111] border border-gray-200 dark:border-[#333] rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col h-[85vh]">
        <div className="flex justify-between p-5 border-b border-gray-100 dark:border-[#222]">
          <h3 className="font-bold flex gap-2 text-gray-900 dark:text-white"><Code className="text-[#f5e100]"/> Código HTML para Correo</h3>
        ...
        <div className="p-5 flex-1 bg-gray-900 overflow-hidden relative">
          ...
          <button onClick={() => { navigator.clipboard.writeText(html); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className={`absolute bottom-6 right-6 text-white px-6 py-3 rounded-xl shadow-lg flex gap-2 font-bold ${copied ? 'bg-green-600' : 'bg-[#f5e100]'}`}>
                    ...
                    {grupos.map(g => (
                      <div key={g.id} className="flex items-center justify-between bg-white dark:bg-[#0a0a0a] p-3 rounded-lg border border-gray-200 dark:border-[#333]">
                        ...
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => eliminarGrupo(g.id)} className="px-2 py-1 text-yellow-500 hover:bg-yellow-50 rounded"><Trash2 size={12} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
