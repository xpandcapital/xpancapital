    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#111111] border border-gray-200 dark:border-[#333] rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          ...
          {currentTemplateId ? (<div className="grid grid-cols-2 gap-3"><button onClick={() => onSave(false)} disabled={templatesLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 disabled:opacity-50"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 13 17 13"/></svg> Actualizar</button><button onClick={() => onSave(true)} disabled={templatesLoading} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 disabled:opacity-50 text-sm"><Database size={18} /> Guardar Como</button></div>) : (<button onClick={() => onSave(true)} disabled={templatesLoading} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 disabled:opacity-50"><Database size={18} /> {templatesLoading ? 'Guardando...' : 'Guardar en la Nube'}</button>)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#111111] border border-gray-200 dark:border-[#333] rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-5 border-b border-gray-100 dark:border-[#222]"><h3 className="font-bold flex items-center gap-2 text-gray-900 dark:text-white text-lg"><Grid className="text-purple-500"/> Galería de Medios</h3><button onClick={() => { onClose(); mediaCallbackRef.current = null; }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors p-1 rounded-md bg-gray-100 dark:bg-[#222]"><X size={18} /></button></div>
        <div className="flex border-b border-gray-200 dark:border-[#222] bg-gray-50 dark:bg-[#161616]">
...
function SenderForm({ editingSender, setEditingSender, saveSender, setTestResult, testingConnection, testResult, senders }) {
  return (
    <div className="bg-gray-50 dark:bg-[#161616] p-4 rounded-xl border border-gray-200 dark:border-[#333] space-y-4">
      <div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1">Nombre del Remitente</label><input type="text" placeholder="Ej: Marketing BLIS" value={editingSender.nombre || ''} onChange={(e) => setEditingSender({...editingSender, nombre: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-[#444] rounded-lg bg-white dark:bg-[#0a0a0a] text-sm focus:outline-none focus:ring-1 focus:ring-purple-500" /></div><div><label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1">Mostrar como</label><input type="text" placeholder="Ej: Ventas BLIS Corp" value={editingSender.from_name || ''} onChange={(e) => setEditingSender({...editingSender, from_name: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-[#444] rounded-lg bg-white dark:bg-[#0a0a0a] text-sm focus:outline-none focus:ring-1 focus:ring-purple-500" /></div></div>
