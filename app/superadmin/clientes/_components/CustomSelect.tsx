        ...
        {isOpen && (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute left-0 right-0 top-full mt-2 z-[10003] bg-[#0f0f0f] border border-white/10 rounded-2xl overflow-hidden shadow-4xl backdrop-blur-xl min-w-[180px]">
              {options.map((opt, i) => (
                <button key={i} onClick={() => { onChange(opt.value); setIsOpen(false); }} className={`w-full text-left px-5 py-3 text-xs font-bold transition-all hover:bg-blis-red hover:text-white ${value === opt.value ? 'bg-white/5 text-blis-red' : 'text-gray-400'}`}>
                  {opt.label}
                </button>
              ))}
            </motion.div>
