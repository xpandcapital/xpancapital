...
export function useTerminalLogic(props: {
  ...
  const [ticker, setTicker] = useState({ price: 2900.00, changePercent: 0.25 });
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedAuto = localStorage.getItem('blis_autopilot'); if (savedAuto) { const parsed = JSON.parse(savedAuto); if (parsed.active) { setAutoPilot(parsed); setChatMessages((prev: any) => [...prev, { role: 'bot', text: "🟢 **Núcleo Sincronizado**: Reanudando sesión activa de Autopiloto..." }]); } }
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && isMounted) {
      try {
        ...
        if (lightweightPositions.length > 0) { const upserts = lightweightPositions.map((op: any) => ({ id: op.id.toString(), payload: op })); supabase.from('trading_open_positions').upsert(upserts).then(); }
    ...
    const runWelcomeScan = async () => {
      ...
      for (const sym of WATCH_LIST) {
        try {
          ...
          const candleData = raw.map((c: any) => ({ open: parseFloat(c[1]), high: parseFloat(c[2]), low: parseFloat(c[3]), close: parseFloat(c[4]), volume: parseFloat(c[5]) }));
  ...
  useEffect(() => {
    if (!isMounted) return;
    try { const lightweightPositions = openPositions.map(({ candlesAtOpen, ...rest }: any) => rest); localStorage.setItem('blis_open_pos_all', JSON.stringify(lightweightPositions)); } catch (e) { console.warn("Storage quota limit reached for open positions.", e); }
  }, [openPositions, isMounted]);
  useEffect(() => {
    if (!isMounted) return;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedAuto) { const parsed = JSON.parse(savedAuto); if (parsed.active) { setAutoPilot(parsed); try { if (typeof setChatMessages === 'function') { setChatMessages((prev: any) => [...prev, { role: 'bot', type: 'text', content: "🟢 Sincronización completa. Reanudando sesión de trading activa e intentando reconectar con el motor..." }]); } } catch (e) { console.error("Exception in fetchInitialHistory:", e); } } }
      const b = localStorage.getItem('blis_bot_budget'); if (b) setBotBudget(Number(b));
      const l = localStorage.getItem('blis_user_lev'); if (l) setUserLeverage(Number(l));
      const f = localStorage.getItem('blis_free_budget'); if (f) setFreeBudget(f === 'true');
    } catch {}
  ...
  const formatChatTime = (ts: any) => { if (!isMounted || !ts) return '--:--'; return new Date(ts).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }); };
  const formatTableTime = (ts: any) => { if (!isMounted || !ts) return '--:--'; return new Date(ts).toLocaleTimeString('es-CO', { hour12: false }); };
  ...
  const formatTimePassed = (startMs: any) => { if (!isMounted || !startMs) return '--:--'; const passed = now - startMs; if (passed < 0) return '--:--'; const mins = Math.floor(passed / 60000).toString().padStart(2, '0'); const secs = Math.floor((passed % 60000) / 1000).toString().padStart(2, '0'); return `${mins}:${secs}`; };
      ...
      const fetchAndConnect = async () => {
        try {
          ...
          const formatted = data.map((d: any, index: number, arr: any) => {
            const closeVal = parseFloat(d[4]) || 0; const volume = parseFloat(d[5]) || 0; const takerBuyBaseAssetVolume = parseFloat(d[9]) || 0;
          ...
          ws.onmessage = (ev: any) => { if (!isMounted) return; const m = JSON.parse(ev.data); if (m.e === 'kline') { currentPriceRef.current = parseFloat(m.k.c); setCandles(prev => { if (prev.length === 0) return prev; const base = (prev[prev.length-1].time === m.k.t) ? prev.slice(0, -1) : prev; const newC = { time: m.k.t, open: parseFloat(m.k.o), high: parseFloat(m.k.h), low: parseFloat(m.k.l), close: parseFloat(m.k.c), volume: parseFloat(m.k.v), takerBuyBaseAssetVolume: parseFloat(m.k.V) }; const full = [...base, newC]; const sma15 = full.length >= 15 ? full.slice(-15).reduce((s, v) => s + v.close, 0) / 15 : null; const sma50 = full.length >= 50 ? full.slice(-50).reduce((s, v) => s + v.close, 0) / 50 : null; const finalArray = [...base, { ...newC, sma15, sma50 }].slice(-500); candlesRef.current = finalArray; return finalArray; }); } else if (m.e === 'aggTrade') { setTicker(prev => ({ ...prev, price: parseFloat(m.p) })); currentPriceRef.current = parseFloat(m.p); } };
          ...
          try {
            ...
            const formatted = proxyData.map((d: any, index: number, arr: any) => { const closeVal = parseFloat(d[4]) || 0; const volume = parseFloat(d[5]) || 0; const takerBuyBaseAssetVolume = parseFloat(d[9]) || 0; const sma15 = index >= 14 ? arr.slice(index - 14, index + 1).reduce((s: any, v: any) => s + (parseFloat(v[4])||0), 0) / 15 : null; const sma50 = index >= 49 ? arr.slice(index - 49, index + 1).reduce((s: any, v: any) => s + (parseFloat(v[4])||0), 0) / 50 : null; return { time: d[0], open: parseFloat(d[1]), high: parseFloat(d[2]), low: parseFloat(d[3]), close: closeVal, volume, takerBuyBaseAssetVolume, sma15, sma50 }; });
  ...
  const chartMath = useMemo(() => {
    ...
    const sma15Path = visibleCandles.length > 0 ? visibleCandles.map((c, i) => { const y = getY(c.sma15); if (!Number.isFinite(y)) return ""; return (i === 0 ? "M " : "L ") + getX(i) + " " + y; }).join(" ") : "";
    const sma50Path = visibleCandles.length > 0 ? visibleCandles.map((c, i) => { const y = getY(c.sma50); if (!Number.isFinite(y)) return ""; return (i === 0 ? "M " : "L ") + getX(i) + " " + y; }).join(" ") : "";
  ...
  const stopAutoPilotManual = (soft = true) => { if (!autoPilot.active) return; const sId = autoPilot.sessionId; if (soft) { setAutoPilot((prev: any) => ({ ...prev, scanningStopped: true })); setChatMessages((prev: any) => [...prev, { role: 'bot', text: `🛑 **Escáner Detenido.** El motor ya no abrirá nuevas posiciones. Gestionará el cierre óptimo de las operaciones restantes...`, timestamp: Date.now() }]); } else { setAutoPilot((prev: any) => ({ ...prev, active: false, expiresAt: null, totalBudget: 0, leverage: 1, sessionId: null, mode: 'SCALPING', scanningStopped: false, riskLevel: 'NORMAL' })); setPendingReportSessionId(sId); setChatMessages((prev: any) => [...prev, { role: 'bot', text: `🏁 **Sesión Finalizada Forzosamente.** Generando reporte consolidado...`, timestamp: Date.now() }]); } };

  const closeSessionTrades = (sId: any, reason: any) => { const cp = currentPriceRef.current; const toClose = openPositionsRef.current.filter(p => p.sessionId === sId || !sId); if (toClose.length === 0) return; let realReturn = 0; let paperReturn = 0; const closed = toClose.map(pos => { const pnl = getPnlData(pos).value; const returnAmount = (pos.amount || 0) + pnl; if (pos.tradeMode === 'REAL') realReturn += returnAmount; else paperReturn += returnAmount; return { ...pos, status: 'CLOSED', closePrice: cp, closeReason: reason, closeTime: Date.now(), finalPnl: pnl, finalPnlPercent: pos.amount ? (pnl / pos.amount) * 100 : 0 }; }); setOpenPositions((prev: any[]) => prev.filter(p => !sId || p.sessionId !== sId)); setTradeHistory((h: any[]) => [...closed, ...h].slice(0, 500)); setHistoryWindow((prev: any[]) => [...closed, ...prev].slice(0, 500)); setHistoryTotal((prev: number) => prev + closed.length); if (realReturn !== 0) { setBalance((b: number) => b + realReturn); balanceRef.current += realReturn; } if (paperReturn !== 0) { setPaperBalance((b: number) => b + paperReturn); paperBalanceRef.current += paperReturn; } };
  ...
  useEffect(() => {
    ...
    const runScan = async () => {
      try {
        ...
        const data = await res.json();
        const cands = data.map((d: any) => ({ close: parseFloat(d[4]) || 0, volume: parseFloat(d[5]) }));
        ...
        const recentTraumas = aiKnowledgeRef.current.filter((k: any) => k.symbol === targetTick.symbol && k.outcome === 'LOSS' && Date.now() - k.timestamp < 45 * 60000);
        ...
        if (bgScore >= MIN_SCORE) {
          ...
          if (tradeModeRef.current === 'PAPER') {
            ...
            if (paperBalanceRef.current >= amountLocal) {
              ...
              let tacticsReason = trendUpBg ? 'Continuación alcista con volumen y estructura alineada' : 'Continuación bajista con volumen y estructura alineada';
              ...
              if (notifsRef.current) { setChatMessages((prev: any[]) => [...prev, { role: 'bot', text: `⚡ **Zero-Lag Sniper**: Entrada **${actionType}** en **${targetTick.symbol.replace('USDT','')}** a $${realEntryPrice.toFixed(4)}.\n🧬 **Motor Lógico**: ${tacticsReason}`, timestamp: Date.now() }]); }
            }
          }
  ...
  useEffect(() => {
    ...
    const aiEngine = setInterval(() => {
      ...
      if (autoPilot.active && !autoPilot.scanningStopped && autoPilot.expiresAt && nowTs > autoPilot.expiresAt) { setAutoPilot((prev: any) => ({ ...prev, scanningStopped: true })); setChatMessages((prev: any) => [...prev, { role: 'bot', text: `⏱️ **Tiempo de Sesión Agotado.** Deteniendo la Inteligencia de Autoaprendizaje.`, timestamp: Date.now() }]); }
      ...
      const traumasRecientes = aiKnowledgeRef.current.filter((m: any) => m.outcome === 'LOSS' && m.symbol === activeSymbol && m.type === decisionAction && nowTs - m.timestamp < 45 * 60000);
      if (traumasRecientes.length > 0) { const penalty = traumasRecientes.length * 3; const dynamicMin = MIN_SCORE + penalty; const actualScore = decisionAction === 'BUY' ? buyScore : sellScore; if (actualScore < dynamicMin) { if (notifsRef.current && nowTs - lastCurrentPriceSymbolRef.current > 30000) { lastCurrentPriceSymbolRef.current = nowTs; setChatMessages((prev: any[]) => [...prev, { role: 'bot', text: `🧠 **Precaución Cognitiva**: Rechacé entrar en ${activeSymbol.replace('USDT','')}. He perdido ${traumasRecientes.length} vez(es) recientemente intentando un ${decisionAction} aquí.`, timestamp: Date.now() }]); } return; } }
      ...
      if (isAuto) {
        ...
        if (notifsRef.current) { setChatMessages((prev: any[]) => [...prev, { role: 'bot', text: `⚡ **${decisionAction} [${autoPilot.mode || 'SCALPING'}]**\n${decisionReasoning}\n\nMONTO IMPRESO: $${useAmount.toFixed(2)} | X${currentLev}`, id: `bot_notif_${nowTs}`, timestamp: nowTs }]); }
    ...
    const monitorEngine = setInterval(async () => {
      ...
      currentPositions.forEach(pos => {
        ...
        if (reason) {
          const lastFailTime = (pos as any)._lastCloseFail || 0; if (Date.now() - lastFailTime < 30000) return;
          closeTradeManual(pos.id, undefined, reason);
          if ((pos as any)._autoReverse && tradeModeRef.current === 'PAPER') {
            setTimeout(() => {
              const revType = (pos as any)._autoReverse; const amountLocal = pos.amount; const localLev = pos.leverage;
              if (paperBalanceRef.current >= amountLocal) {
                ...
                setOpenPositions(prev => [...prev, newPos]); if (notifsRef.current) { setChatMessages((prev: any[]) => [...prev, { role: 'system', text: `🔄 **SAR**: Estructura rota en ${pos.symbol.replace('USDT','')}. Inversión a **${revType}**.`, timestamp: Date.now() }]); }
              }
            }, 500);
          }
          ...
          setAiKnowledge((prev: any[]) => { const u = [newMem, ...prev]; aiKnowledgeRef.current = u; return u; });
  ...
  useEffect(() => { const handleMouseOver = (e: any) => { const el = e.target?.closest ? e.target.closest('[data-tooltip]') : null; if (el) { const rect = el.getBoundingClientRect(); const text = el.getAttribute('data-tooltip'); if (text) setGlobalTooltip({ show: true, text, x: rect.right + 12, y: Math.max(10, rect.top + (rect.height / 2)) }); } else setGlobalTooltip(null); }; window.addEventListener('mouseover', handleMouseOver); return () => window.removeEventListener('mouseover', handleMouseOver); }, []);

  const currentUsedMargin = openPositions.reduce((sum, p) => p.sessionId === autoPilot.sessionId ? sum + p.amount : sum, 0);
  useEffect(() => { if (strategySuggestions) { setManualStrategy((prev: any) => ({ ...prev, ...strategySuggestions })); setStrategySuggestions(null); } }, [strategySuggestions]);
  ...
  const closeTradeManual = async (id: string | number, e?: React.MouseEvent, autoReason?: string) => {
    ...
    try {
      ...
      if (dataSource === 'binance' && pos.tradeMode === 'REAL') {
        ...
        if (posQty > 0) {
          ...
          } catch (err: any) { setChatMessages((prev: any) => [...prev, { id: Date.now() + 1, role: 'bot', text: `❌ **Error liquidando (Binance)**: ${err.message}. SE FORZARÁ EL CIERRE EN LA INTERFAZ.`, timestamp: Date.now() }]); }
        }
      }
      ...
      setTradeHistory((h: any[]) => [closed as any, ...h].slice(0, 500));
    ...
    } catch (e: any) { console.error("Error crítico al cerrar posición", e); setGlobalAlert(`Error interno al cerrar: ${e.message}`); }
  };
  ...
  const executeSignal = async (msg: any): Promise<boolean> => {
    ...
    const openPaperAmt = openPositionsRef.current.filter((p: any) => p.tradeMode === 'PAPER').reduce((s: number, p: any) => s + (p.amount || 0), 0);
    ...
    if (lev === 0) { const risk = autoPilot.riskLevel || 'NORMAL'; lev = botMode === 'SCALPING' ? (risk === 'TURBO' ? 50 : (risk === 'HIGH' ? 30 : 20)) : 10; const nowTs = Date.now(); if (nowTs - (msg.signalData._lastLevLog || 0) > 20000) { setChatMessages(prev => [...prev, { role: 'bot', text: `🧠 **OVERCLOCK APALANCADO**: Operando en Inteligencia AUTO. He inyectado un multiplicador Dinámico e Institucional de **x${lev}** para arrasar con el mercado.`, timestamp: Date.now() }]); msg.signalData._lastLevLog = nowTs; } }
    ...
    if (dataSource === 'binance' && tradeMode === 'REAL') {
      try {
        ...
        const params: any = { symbol: activeSymbol, side: actionType, type: 'MARKET' }; params.quantity = finalQty;
        if (parseFloat(finalQty) <= 0) { setChatMessages((prev: any) => [...prev, { id: Date.now(), role: 'bot', text: `⚠️ **Monto muy bajo.**`, timestamp: Date.now() }]); setManualExecStatus({ text: 'Monto muy bajo para Binance.', type: 'error' }); return false; }
        ...
        if (!res.ok) {
          ...
          setChatMessages((prev: any) => [...prev, { id: Date.now(), role: 'bot', text: `⚠️ **BINANCE RECHAZÓ LA ORDEN**: ${data.error || 'Error de conexión o balance insuficiente.'}`, timestamp: Date.now() }].slice(-50));
        ...
        setManualExecStatus({ text: `Error de red: ${err.message}`, type: 'error' }); return false;
      }
    }
    ...
    const tradeData: any = { id: orderIdRef, symbol: msg.signalData.symbol || activeSymbol || 'BTCUSDT', sessionId: autoPilot.active ? autoPilot.sessionId : null, tradeMode, type: msg.signalData.type, entryPrice: cp, amount: amt, leverage: lev, fee, status: 'OPEN', openTime: Date.now(), mode: botMode, explanation: msg.signalData.reason, isManual: isManualTrade, openedBy: isManualTrade ? 'HUMANO' : 'IA', closedBy: null, candlesAtOpen: candleSnapshot, targetPrice: isManualTrade ? null : (msg.signalData.targetPrice || null), stopPrice: isManualTrade ? null : (msg.signalData.stopPrice || null), binanceQty };
    setOpenPositions((prev: any[]) => [tradeData, ...prev]); openPositionsRef.current = [tradeData, ...openPositionsRef.current];
  ...
  const executeManualSignal = async (type: 'BUY' | 'SELL', customAmt?: number) => {
    ...
    if (type === 'BUY') { if (amountToUse < 10) { const msg = 'Monto Mínimo: Se requieren al menos $10 para comprar.'; setChatMessages((prev: any) => { const lastMsg = prev[prev.length - 1]; if (lastMsg && lastMsg.text === msg) return prev; return [...prev, { role: 'bot', text: `⚠️ **${msg}**`, timestamp: Date.now() }].slice(-50); }); setManualExecStatus({ text: msg, type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } if (amountToUse > activeBal) { setChatMessages((prev: any) => [...prev, { role: 'bot', text: `⚠️ **Fondo Insuficiente.** Disponible: ${fmtUsd(activeBal)}`, timestamp: Date.now() }]); setManualExecStatus({ text: `Fondo Insuficiente. Disponible: ${fmtUsd(activeBal)}`, type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } } else { if (amountToUse < 10) { setChatMessages((prev: any) => [...prev, { role: 'bot', text: `⚠️ **Monto Mínimo**: Se requieren al menos $10 para abrir Short (Venta).`, timestamp: Date.now() }]); setManualExecStatus({ text: 'Monto Mínimo: $10 para Short.', type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } if (amountToUse > activeBal) { setChatMessages((prev: any) => [...prev, { role: 'bot', text: `⚠️ **Margen Insuficiente para Venta.** Disponible: ${fmtUsd(activeBal)}`, timestamp: Date.now() }]); setManualExecStatus({ text: `Margen Insuficiente. Disponible: ${fmtUsd(activeBal)}`, type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } }
  ...
  const handleSendMessage = (e: any) => { executeSendPrompt(e); };

  const executeSendPrompt = async (e: any, overrideText: string | null = null) => {
    ...
    if (!keys.gemini && !keys.openai) { setChatMessages((prev: any) => [...prev, { role: 'bot', text: "⚠️ **Configuración Requerida**: Conecta tu API Key de Gemini o ChatGPT en el panel de **APIs & Cloud** para habilitar la inteligencia evolutiva.", timestamp: Date.now() }]); setIsTyping(false); return; }
    try {
