...
export function useTerminalLogic(props: {
  ...
  useEffect(() => {
    ...
    const fetchInitialHistory = async () => {
      try {
        ...
        if (modeFilter) { pageQuery = (pageQuery as unknown as SupaFilterable).eq('trade_mode', modeFilter); fullQuery = (fullQuery as unknown as SupaFilterable).eq('trade_mode', modeFilter); }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedAuto = localStorage.getItem('blis_autopilot'); if (savedAuto) { const parsed = JSON.parse(savedAuto); if (parsed.active) { setAutoPilot(parsed); setChatMessages(prev => [...prev, { role: 'bot', text: "🟢 **Núcleo Sincronizado**: Reanudando sesión activa de Autopiloto..." }]); } }
          ...
          if (pos._autoReverse && tradeModeRef.current === 'PAPER') {
            setTimeout(() => {
              ...
              if (paperBalanceRef.current >= amountLocal) {
                ...
                setOpenPositions(prev => [...prev, newPos]); if (notifsRef.current) { setChatMessages((prev: ChatMessage[]) => [...prev, { role: 'system', text: `🔄 **SAR**: Estructura rota en ${pos.symbol.replace('USDT','')}. Inversión a **${revType}**.`, timestamp: Date.now() }]); }
              }
            }, 500);
          }
  ...
  const closeTradeManual = async (id: string | number, e?: React.MouseEvent, autoReason?: string) => {
    ...
    try {
      ...
      setHistoryWindow((prev: TradeHistoryEntry[]) => [closed, ...prev].slice(0, 500));
    ...
    if (dataSource === 'binance' && tradeMode === 'REAL') {
      try {
        ...
        if (!res.ok) {
          ...
          setChatMessages(prev => [...prev, { id: Date.now(), role: 'bot', text: `⚠️ **BINANCE RECHAZÓ LA ORDEN**: ${data.error || 'Error de conexión o balance insuficiente.'}`, timestamp: Date.now() }].slice(-50));
  ...
  const executeManualSignal = async (type: 'BUY' | 'SELL', customAmt?: number) => {
    ...
    if (type === 'BUY') { if (amountToUse < 10) { const msg = 'Monto Mínimo: Se requieren al menos $10 para comprar.'; setChatMessages(prev => { const lastMsg = prev[prev.length - 1]; if (lastMsg && lastMsg.text === msg) return prev; return [...prev, { role: 'bot', text: `⚠️ **${msg}**`, timestamp: Date.now() }].slice(-50); }); setManualExecStatus({ text: msg, type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } if (amountToUse > activeBal) { setChatMessages(prev => [...prev, { role: 'bot', text: `⚠️ **Fondo Insuficiente.** Disponible: ${fmtUsd(activeBal)}`, timestamp: Date.now() }]); setManualExecStatus({ text: `Fondo Insuficiente. Disponible: ${fmtUsd(activeBal)}`, type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } } else { if (amountToUse < 10) { setChatMessages(prev => [...prev, { role: 'bot', text: `⚠️ **Monto Mínimo**: Se requieren al menos $10 para abrir Short (Venta).`, timestamp: Date.now() }]); setManualExecStatus({ text: 'Monto Mínimo: $10 para Short.', type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } if (amountToUse > activeBal) { setChatMessages(prev => [...prev, { role: 'bot', text: `⚠️ **Margen Insuficiente para Venta.** Disponible: ${fmtUsd(activeBal)}`, timestamp: Date.now() }]); setManualExecStatus({ text: `Margen Insuficiente. Disponible: ${fmtUsd(activeBal)}`, type: 'error' }); setTimeout(() => setManualExecStatus(null), 10000); return; } }
