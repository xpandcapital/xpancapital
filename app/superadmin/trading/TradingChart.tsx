...
const NEON_GREEN  = '#0ecb81'; // Brand emerald — matches Blis Corp web
...
export const TradingChart = ({ 
  ...
  isDragging?: boolean,
  currentDrawing?: any,
  hoverData?: { x: number, y: number, price: number, time: number } | null,
  ...
  const RULER_X = dims.width - (padding?.right ?? 80);  // x where price ruler starts
  const RULER_W = padding?.right ?? 80;

  // Adaptive step count: more labels when chart is taller, fewer when many candles are visible (zoomed out)
  const baseSteps = Math.max(15, Math.floor(dims.height / 28)); // ~1 label every 28px
  const priceStepCount = Math.min(40, baseSteps);
  const stepSize = priceRange / priceStepCount;
  // Density reduction when zoomed out (many candles = skip labels to avoid saturation)
  const candleCount = visibleCandles?.length ?? 0;
  const labelSkip = candleCount > 150 ? 4 : candleCount > 80 ? 2 : 1; // show 1 of every N labels

  // ─── OPTIMIZACIÓN HFT: RENDERIZADO ESTÁTICO MEMOIZADO ───
    ...
    <>
      ...
      <g transform={`translate(${RULER_X}, 0)`}>
        ...
        {Array.from({ length: priceStepCount + 1 }).map((_, i) => {
          // Adaptive: skip labels based on zoom level
          ...
          return (
            <g key={`ruler-${i}`}>
              ...
              />
              {/* Price label — only on major ticks */}
              {isMajor && (
                <text
                  x={11} y={y + 4}
      ...
      {/* ─── GRID DE PRECISIÓN MILIMÉTRICA ─── */}
      {showGrid && (
        <g opacity={0.07}>
          {/* Líneas Horizontales (Precios Clave) */}
      ...
      {/* ─── IA PROJECTION (ESTIMACIÓN DE OBJETIVO) ─── */}
      <g clipPath="url(#candleArea)">
        {(() => {
          // Sistema anti-superposición: recolectar todos los labels y ajustar posiciones
          ...
          // Ordenar por posición Y y separar labels que se superponen (mínimo 22px entre cada uno)
          ...
          for (let i = 1; i < allLabels.length; i++) {
            ...
            if (curr.y - prev.y < MIN_GAP) {
              curr.y = prev.y + MIN_GAP; // Empujar hacia abajo
            }
          }
      ...
      {showSma && (
        <>
            ...
            {/* HUD: LEYENDA TÉCNICA (TOP LEFT) - Flotante sin fondo y pegada a la regla */}
      ...
      {/* ─── ZONAS DE LIQUIDEZ INSTITUCIONAL (FVG) ─── */}
      <g clipPath="url(#candleArea)">
      ...
      {/* ─── LIBRO DE ÓRDENES (DOM - Depth of Market) ─── */}
      {showDom && (
        <g clipPath="url(#candleArea)">
           {Array.from({ length: 50 }).map((_, i) => {
               ...
               // Generación paramétrica para aparentar liquidez real concentrada cerca del precio actual
               const distFromPrice = Math.abs(priceLvl - cp);
               const weight = Math.max(0.1, 1 - (distFromPrice / (pRange * 0.3))); // Más volumen cerca del precio
               // Añadir ruido persistente basado en el precio
        ...
        {positions.map((p: any) => {
          ...
          return (
            ...
            >
              ...
              {/* TP line + Flecha de proyección del trade (dirección visual correcta) */}
              {showAiZonesUI && targetY && Number.isFinite(targetY) && (
                <g>
                  ...
                  {/* Flecha de proyección dirección: BUY = verde↑, SELL = rojo↓ */}
                  {(() => {
                    const arrowX = 60;
                    const arrowCol = p.type === 'BUY' ? NEON_GREEN : NEON_RED;
                    // Línea vertical desde entry hacia TP
                    return (
                      <g opacity={0.65}>
                        <line
                          x1={arrowX} y1={y}
              ...
              {/* Compact badge pinned to the LEFT border of the ruler — no longer floating over candles */}
              <g>
                {/* Small triangle pointer at ruler edge */}
                <polygon
                  points={`${RULER_X - 6},${y} ${RULER_X},${y - 5} ${RULER_X},${y + 5}`}
                  fill={isHovered ? NEON_YELLOW : col}
                  opacity={isHovered ? 1 : 0.85}
                />
                {/* Pill badge on the ruler itself */}
                <rect
                  x={RULER_X} y={y - 10}
  ...
  ), [chartMath, drawings, currentDrawing, positions, ticker?.price, aiZones, hoverPositionId, selectedPositionId, dims.height, dims.width, RULER_X, RULER_W, minPrice, maxPrice, visibleCandles, candleWidth, padding, sma15Path, sma50Path, priceStepCount, stepSize, labelSkip, setHoverPositionId, setSelectedPositionId, cp, showGrid, showSma, showAiZonesUI, showPositionLines]);
  ...
  return (
    <svg width={dims.width} height={dims.height} className="select-none bg-[#0b0e11]">
      ...
      {/* ─── LÍNEA PROYECTADA DEL PRECIO ACTUAL (sobre el gráfico) ─── */}
      {(() => {
        const y = getY(cp);
        if (!Number.isFinite(y)) return null;
        return (
          <g style={{ pointerEvents: 'none' }}>
            {/* Línea blanca proyectada */}
            <line
      ...
      {tradeReplayData && tradeReplayData.candles.length > 0 && (() => {
        ...
        // Overlay dimensions - use most of chart area
        const ox = 40;
        const oy = 35;
        const ow = RULER_X - 60;
        ...
        return (
          <g style={{ pointerEvents: 'none' }}>
            ...
            {/* Entry/exit arrows connecting the two lines */}
