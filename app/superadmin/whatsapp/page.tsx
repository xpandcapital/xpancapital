...
const SELECT_CLS = "w-full bg-black/50 border border-white/10 rounded-lg px-3 py-3 text-sm text-white mt-1 appearance-none cursor-pointer focus:outline-none focus:border-emerald-500/50 transition-all"
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {showForm && (
          <div className="bg-zinc-900/50 border border-emerald-500/20 rounded-2xl p-6 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  <SearchableSelect
                    options={msgTemplates.map((t: any) => ({ value: t.id, label: `${t.nombre} (${t.categoria || 'General'})` }))}
                    value=""
                    onChange={v => { if (v) loadMsg(msgTemplates.find(t => t.id === v)); }}
              ...
              {messageGroups.map((g, gIdx) => (
                <div key={gIdx} className="bg-black/30 border border-white/5 rounded-xl p-3 mb-2 space-y-2">
                  ...
                  <textarea
                    ...
                    data-group-idx={gIdx}
                    rows={5} className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-3 text-base text-white resize-none"
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  <div className="relative">
                  <SearchableSelect
                    options={varTemplates.map((t: any) => ({ value: t.id, label: `${t.nombre} (${t.categoria || 'General'})` }))}
                    value=""
                    onChange={v => { if (v) loadVars(varTemplates.find(t => t.id === v)); }}
                    placeholder="Cargar plantilla..."
                    searchPlaceholder="Buscar..."
                  />
                  </div>
            ...
            {contactMode === "leads" && <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-gray-400 uppercase tracking-widest font-bold">Estado</label><SearchableSelect options={[{value:"",label:"Todos"},{value:"nuevo",label:"Nuevo"},{value:"contactado",label:"Contactado"},{value:"calificado",label:"Calificado"},{value:"cliente",label:"Cliente"}]} value={leadEstado} onChange={setLeadEstado} placeholder="Todos" /></div><div><label className="text-xs text-gray-400 uppercase tracking-widest font-bold">Campaña</label><SearchableSelect options={campanasList.map((c: any) => ({ value: c.id, label: c.nombre }))} value={leadCampanaId} onChange={setLeadCampanaId} placeholder="Todas" /></div></div>}
