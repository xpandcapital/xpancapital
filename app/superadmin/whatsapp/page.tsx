      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        {showForm && (
          <div className="bg-zinc-900/50 border border-emerald-500/20 rounded-2xl p-6 space-y-5">
            ...
            {contactMode === "leads" && <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-gray-400 uppercase tracking-widest font-bold">Estado</label><select value={leadEstado} onChange={e => setLeadEstado(e.target.value)} className={SELECT_CLS}><option value="">Todos</option><option value="nuevo">Nuevo</option><option value="contactado">Contactado</option><option value="calificado">Calificado</option><option value="cliente">Cliente</option></select></div><div><label className="text-xs text-gray-400 uppercase tracking-widest font-bold">Campaña</label><SearchableSelect options={campanasList.map((c: any) => ({ value: c.id, label: c.nombre }))} value={leadCampanaId} onChange={setLeadCampanaId} placeholder="Todas" /></div></div>}
