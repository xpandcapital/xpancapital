...
import {
  ScrollText, Trash2, RefreshCw, Globe, Route, Server, Zap, Eye, Filter, Bot, Info, Search, X
} from 'lucide-react';
...
const PAISES: Record<string, string> = {
  ...
  KZ: "Kazajistan", UZ: "Uzbekistan", MN: "Mongolia",
...
export function AccessLogsTool() {
  ...
  const clearLogs = async () => {
    ...
    try {
      ...
      setLogs([]); setStats(null); fetchData();
    } catch { /* */ }
  ...
  const filteredLogs = useMemo(() => {
    let result = logs;
    if (ocultarBots) result = result.filter(log => !BOT_SCAN_PATTERNS.some(p => log.ruta.toLowerCase().includes(p.toLowerCase())));
    if (filtroRuta) {
      const q = filtroRuta.toLowerCase();
      result = result.filter(log => log.ruta.toLowerCase().includes(q));
    }
    return result;
  }, [logs, ocultarBots, filtroRuta]);
  ...
  return (
    <div className="bg-zinc-950 rounded-xl border border-white/5">
      ...
      <div className="px-5 py-3 border-b border-white/5 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-sm font-black uppercase tracking-tight text-white">Logs de Acceso</h2>
          <p className="text-[10px] text-gray-500">Registro de intentos bloqueados</p>
        </div>
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <button onClick={clearLogs} disabled={clearing}
            className="flex items-center gap-1 px-2.5 py-1 bg-red-500/10 text-red-400 rounded-md text-[10px] font-medium hover:bg-red-500/20 disabled:opacity-50">
      ...
      <div className="p-4 space-y-3">
        ...
        {stats && (
          <div className="grid grid-cols-4 gap-2">
            {[
              { icon: Zap, color: 'text-red-400', label: 'Bloqueos hoy', value: stats.total_hoy },
              { icon: Globe, color: 'text-blue-400', label: 'Paises', value: stats.paises_unicos },
              { icon: Server, color: 'text-amber-400', label: 'IPs unicas', value: stats.ips_unicas },
              { icon: Route, color: 'text-green-400', label: 'Rutas atacadas', value: stats.top_rutas?.length || 0 },
            ].map((k, i) => (
            ...
            <div className="flex items-center gap-1.5 mb-2">
              ...
              {stats.pico_hora && (
                <span className="text-[8px] text-gray-600 ml-auto">
                  Pico {stats.pico_hora.hora}h · {stats.pico_hora.count}
                </span>
              )}
            </div>
        ...
        {botScanCount > 0 && (
          <div className="flex items-start gap-2 bg-blue-500/5 border border-blue-500/10 rounded-lg px-3 py-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] text-blue-400 font-bold">{botScanCount} peticiones son escaneos de bots</p>
              <p className="text-[9px] text-blue-400/50 truncate">
                Rutas como wp-includes, .env o xmlrpc.php son bots buscando vulnerabilidades. No son ataques dirigidos.
              </p>
            </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <input type="text" value={filtroRuta} onChange={e => setFiltroRuta(e.target.value)}
            placeholder="Filtrar ruta..." className="flex-1 bg-zinc-900 rounded-md px-2 py-1 text-[10px] text-white placeholder-gray-500 outline-none border border-white/5 min-w-[120px]" />
          {filtroRuta && (
            <button onClick={() => setFiltroRuta('')} className="text-gray-500 hover:text-white">
              <X className="w-3 h-3" />
            </button>
          )}
          <span className="text-[9px] text-gray-600">{filteredLogs.length} registros</span>
        </div>
            ...
            <tbody>
              {filteredLogs.length === 0 && !loading && (
                <tr><td colSpan={6} className="text-center py-8 text-gray-600">
                  {stats?.total_hoy === 0 ? 'Sin bloqueos hoy' : ocultarBots ? 'Solo bots filtrados' : 'Sin resultados'}
                </td></tr>
              )}
              {filteredLogs.map((log, i) => (
                <tr key={log.id || i} className="border-b border-white/5 hover:bg-white/[0.02]">
                  ...
                  <td className="py-1.5 px-2 text-gray-400 font-mono text-[9px] truncate max-w-[280px]" title={log.ruta}>
                    {log.ruta}
                  </td>
                  <td className="py-1.5 px-2">
