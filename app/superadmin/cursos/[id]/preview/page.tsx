...
import {
    ArrowLeft, GraduationCap, ChevronDown, ChevronRight, MonitorPlay, FileText,
    CheckCircle2, Loader2, BookOpen, ListChecks, Circle, X
} from 'lucide-react'
...
export default function CursoPreviewPage() {
    ...
    useEffect(() => {
        const fetchCurso = async () => {
            setLoading(true)
            try {
                const res = await fetch(`/api/admin/cursos?id=${cursoId}`)
                const data = await res.json()
                if (data.success && data.data) {
                    const c = data.data
                    setCurso(c)
                    if (c.modulos) {
                        setOpenModules(new Set(c.modulos.slice(0, 2).map((m: Module) => m.id)))
                        setActiveModule(c.modulos[0]?.id || null)
                        if (c.modulos[0]?.lessons?.[0]) {
                            setActiveLesson(c.modulos[0].lessons[0])
                        }
                    }
                }
            } catch {}
    ...
    const navigateLesson = (direction: 'prev' | 'next') => {
        if (!curso?.modulos || !activeLesson) return
        const allLessons = curso.modulos.flatMap(m => m.lessons.map(l => ({ lesson: l, moduleId: m.id })))
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === 'Escape') router.push('/superadmin/cursos')
        }
    ...
    if (!curso) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-black gap-4">
                ...
                <button onClick={() => router.push('/superadmin/cursos')} className="text-blis-red text-sm hover:underline">Volver a cursos</button>
            </div>
        )
    }
        ...
        <div className="flex h-screen bg-black overflow-hidden">
            ...
            <div className="w-80 shrink-0 bg-zinc-950 border-r border-white/5 flex flex-col">
                <div className="px-5 py-4 border-b border-white/5 shrink-0">
                    <button onClick={() => router.push('/superadmin/cursos')} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm mb-4 group">
                    ...
                    <div className="flex items-center gap-3">
                        {curso.imagen_principal ? (
                            ...
                            <div className="w-10 h-10 rounded-lg bg-blis-red/10 border border-blis-red/20 flex items-center justify-center"><GraduationCap className="w-5 h-5 text-blis-red" /></div>
                        )}
                        <div className="min-w-0 flex-1">
                            <h2 className="text-white font-bold text-sm truncate">{curso.nombre}</h2>
                            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{totalLessons} lecciones</p>
                        </div>
                    </div>
                    <div className="mt-2 flex items-center gap-2">
                        {curso.category && <span className="text-[9px] font-bold px-2 py-0.5 rounded bg-blis-red/10 text-blis-red border border-blis-red/20 uppercase tracking-widest">{curso.category}</span>}
                        <span className={`text-[9px] font-bold px-2 py-0.5 rounded uppercase tracking-widest ${curso.status === 'Publicado' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>{curso.status || 'Borrador'}</span>
                        {curso.para_equipo && <span className="text-[9px] font-bold px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 uppercase tracking-widest">Equipo</span>}
                    </div>
                </div>
                    ...
                    {modulos.map((modulo, mIdx) => {
                        ...
                        return (
                            <div key={modulo.id} className="border-b border-white/[0.03]">
                                <button onClick={() => toggleModule(modulo.id)} className="w-full flex items-center gap-3 px-5 py-3 hover:bg-white/[0.02] transition-colors">
                                    <div className="w-6 h-6 rounded-md text-[10px] font-black flex items-center justify-center shrink-0 bg-white/5 text-gray-500 border border-white/5">
                                        {mIdx + 1}
                                    </div>
                                    <div className="flex-1 text-left min-w-0">
                                        <p className="text-white text-xs font-bold truncate">{modulo.title}</p>
                                        <p className="text-[9px] text-gray-600">{moduleLessons.length} lecciones</p>
                                    </div>
                                ...
                                {isOpen && moduleLessons.length > 0 && (
                                    <div className="pb-2">
                                        {moduleLessons.map((lesson) => {
                                            ...
                                            return (
                                                <button key={lesson.id} onClick={() => selectLesson(lesson, modulo.id)} className={`w-full flex items-center gap-3 px-5 py-2.5 transition-all text-left group ${isActive ? 'bg-blis-red/5 border-l-2 border-l-blis-red' : 'hover:bg-white/[0.02] border-l-2 border-l-transparent'}`}>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                <div className="shrink-0 px-6 py-3 bg-zinc-950/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-bold px-3 py-1 rounded-lg bg-blis-red/10 text-blis-red border border-blis-red/20 uppercase tracking-widest">Vista Previa</span>
                </div>
                    ...
                    {activeLesson ? (
                        <div className="max-w-4xl mx-auto w-full">
                            ...
                            <div className="p-8 md:p-12 space-y-8">
                                ...
                                {activeLesson.type === 'video' && <h2 className="text-xl font-black text-white">{activeLesson.title}</h2>}
                                ...
                                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => navigateLesson('prev')} disabled={!hasPrev} className="px-4 py-2.5 bg-white/5 border border-white/5 rounded-xl text-white text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all disabled:opacity-30 disabled:cursor-not-allowed">Anterior</button>
                                        <button onClick={() => navigateLesson('next')} disabled={!hasNext} className="px-4 py-2.5 bg-blis-red rounded-xl text-white text-[10px] font-bold uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-lg shadow-blis-red/20">Siguiente</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-center h-full">
                            <div className="text-center">
                                ...
                                <p className="text-gray-600 text-sm mt-1">Elige una lección del menú lateral</p>
                            </div>
                        </div>
                    )}
                </div>
