...
import {
    ...
    Banknote, CreditCard, ArrowUpRight
} from "lucide-react";
...
export default function VentasAdminPage() {
    ...
    const [formNueva, setFormNueva] = useState({ user_email: "", producto_id: "", metodo_pago: "transferencia", monto_usd: 0 });
    ...
    // Cargar clientes y productos para el modal
    useEffect(() => {
    ...
    const registrarVentaOffline = async () => {
        ...
        const res = await fetch("/api/admin/ventas", {
            ...
            body: JSON.stringify({
                ...
                monto_usd: formNueva.monto_usd || 0,
            }),
        });
        if (res.ok) {
            setModalNueva(false);
            setFormNueva({ user_email: "", producto_id: "", metodo_pago: "transferencia", monto_usd: 0 });
            await cargar();
        }
        setGuardando(false);
    };
    ...
    // Stats
    const completados = ventas.filter(v => v.estado === "completado").length + total > 0 ? 0 : 0;
    ...
    const estadoBadge = (estado: string) => {
        const map: Record<string, { color: string; label: string; icon: any }> = {
            ...
            reembolsado: { color: "bg-blue-500/10 text-blue-400 border-blue-500/20", label: "Reembolsado", icon: Clock },
        };
        ...
        <div className="min-h-screen bg-black text-white p-6">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <Button onClick={() => setModalNueva(true)} className="bg-emerald-600 hover:bg-emerald-600/90 text-white font-black uppercase tracking-wider text-xs">
                        <Plus className="w-4 h-4 mr-2" /> Registrar Venta Offline
                    </Button>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-6">
                    <div className="relative flex-1">
                        ...
                        <Input
                            ...
                            className="bg-zinc-900/50 border-white/10 text-white placeholder:text-gray-600 rounded-xl pl-10"
                        />
                    </div>
                    <select
                    ...
                    <Button onClick={handleSearch} className="bg-blis-red hover:bg-blis-red/90 text-white text-xs">
                                ...
                                <tbody className="divide-y divide-white/[0.02]">
                                    {ventas.map(venta => (
                                        <tr key={venta.id} className="hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="p-4">
                                                <p className="text-sm text-gray-300 truncate max-w-[200px]">{venta.producto?.nombre || "Producto"}</p>
                                                <span className="text-[10px] text-gray-600">{venta.producto?.categoria?.nombre || venta.producto?.tipo || ""}</span>
                                            </td>
                                            <td className="p-4 hidden md:table-cell">
                                                <span className="flex items-center gap-1 text-xs text-gray-400">
                                                    {venta.metodo_pago === 'transferencia' ? <Banknote className="w-3 h-3" /> : <CreditCard className="w-3 h-3" />}
                                            ...
                                            <td className="p-4">
                                                <div className="flex items-center gap-1">
                                                    {venta.estado === "pendiente" && (
                                                        <button
                                                            onClick={() => actualizarEstado(venta.id, "completado")}
                                                    ...
                                                    {venta.estado === "completado" && (
                                                        <button
                                                            onClick={() => actualizarEstado(venta.id, "reembolsado")}
                ...
                {total > 50 && (
                    <div className="flex justify-center gap-2 mt-6">
                        <Button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} variant="outline" className="border-white/10 text-gray-400">
                            Anterior
                        </Button>
                        <span className="px-4 py-2 text-sm text-gray-400">Página {page} de {Math.ceil(total / 50)}</span>
                        <Button onClick={() => setPage(p => p + 1)} disabled={page * 50 >= total} variant="outline" className="border-white/10 text-gray-400">
                            Siguiente
                        </Button>
                    </div>
                )}
            ...
            <Dialog open={modalNueva} onOpenChange={setModalNueva}>
                <DialogContent className="bg-[#111] border-white/10 text-white max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-sm font-black uppercase tracking-wider">Registrar Venta Offline</DialogTitle>
                        <p className="text-xs text-gray-500 mt-1">Para pagos por transferencia, efectivo u otros medios.</p>
                    </DialogHeader>
                    <div className="space-y-4 mt-4">
                        <div>
                            <label className="text-xs text-gray-400 uppercase font-bold block mb-2">Email del Cliente *</label>
                            <Input
                                value={formNueva.user_email} onChange={e => setFormNueva({ ...formNueva, user_email: e.target.value })}
                                placeholder="cliente@email.com" className="bg-white/5 border-white/10 text-white"
                            />
                            <datalist id="clientes-list">
                                {clientes.map(c => <option key={c.id} value={c.email || ""}>{c.nombre}</option>)}
                            </datalist>
                        </div>
                        <div>
                            <label className="text-xs text-gray-400 uppercase font-bold block mb-2">Producto *</label>
                            <select
                                value={formNueva.producto_id} onChange={e => setFormNueva({ ...formNueva, producto_id: e.target.value })}
                                className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white"
                            >
                                <option value="">Seleccionar producto...</option>
                                {productos.map(p => (
                                    <option key={p.id} value={p.id}>{p.nombre} ({p.categoria?.nombre || p.tipo})</option>
                                ))}
                            </select>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    value={formNueva.metodo_pago} onChange={e => setFormNueva({ ...formNueva, metodo_pago: e.target.value })}
                                    className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white"
                                >
                            ...
                            <div>
                                ...
                                <Input
                                    type="number" value={formNueva.monto_usd || ""} onChange={e => setFormNueva({ ...formNueva, monto_usd: parseFloat(e.target.value) || 0 })}
                                    placeholder="0" className="bg-white/5 border-white/10 text-white"
                                />
                            </div>
                        </div>
                        <Button onClick={registrarVentaOffline} disabled={guardando || !formNueva.user_email || !formNueva.producto_id} className="w-full bg-emerald-600 hover:bg-emerald-600/90 text-white font-bold">
