...
export function NivelForm({ nivel, onSubmit, onCancel }: Props) {
  ...
  const [color, setColor] = useState(nivel?.color || '#f5e100')
      ...
      <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <input value={color} onChange={e => setColor(e.target.value)} className="flex-1 bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#f5e100] focus:outline-none font-mono" />
          </div>
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Ícono SVG (nombre de lucide-react)</label>
          <input value={iconoSvg} onChange={e => setIconoSvg(e.target.value)} placeholder="medal, crown, star..." className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#f5e100] focus:outline-none" />
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Imagen URL (opcional, sobreescribe ícono)</label>
          <input value={imagenUrl} onChange={e => setImagenUrl(e.target.value)} placeholder="https://..." className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#f5e100] focus:outline-none" />
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Puntos requeridos</label>
          <input type="number" value={puntosReq} onChange={e => setPuntosReq(parseInt(e.target.value) || 0)} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#f5e100] focus:outline-none" />
        </div>
