    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-lg space-y-4">
        ...
        <div>
          ...
          <input value={descripcion} onChange={e => setDescripcion(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#ff1e56] focus:outline-none" />
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Tipo</label>
          <select value={tipo} onChange={e => setTipo(e.target.value as LogroFormData['tipo'])} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#ff1e56] focus:outline-none">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input value={iconoSvg} onChange={e => setIconoSvg(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#ff1e56] focus:outline-none" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Imagen URL</label>
            <input value={imagenUrl} onChange={e => setImagenUrl(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#ff1e56] focus:outline-none" />
          </div>
        </div>

        <div>
          <label className="block text-sm text-gray-400 mb-1">Condición (JSON)</label>
          <textarea value={condicionJson} onChange={e => setCondicionJson(e.target.value)} rows={4} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm font-mono focus:border-[#ff1e56] focus:outline-none" placeholder='{"cursos_completados": 5}' />
        ...
        <div>
          ...
          <input type="number" value={puntosBonus} onChange={e => setPuntosBonus(parseInt(e.target.value) || 0)} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:border-[#ff1e56] focus:outline-none" />
        </div>
