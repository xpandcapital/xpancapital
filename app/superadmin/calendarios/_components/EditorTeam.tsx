        ...
        {formData.tipo === 'rotacion' && (
          <div>
            ...
            <select
              ...
              onChange={e => {
                const newConfig = { ...(formData.configuracion as Record<string, unknown>), logic: e.target.value }
                updateField('configuracion', newConfig)
              }}
              className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-2.5 outline-none focus:border-blis-red text-sm"
            >
              <option value="round_robin">Equitativo (Round Robin)</option>
