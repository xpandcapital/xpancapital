                                                        ...
                                                        {bks.length > 0 ? <div className="space-y-3">{bks.map((b: any, i: number) => (
                                                            <div key={i} className="bg-black/30 border border-white/10 rounded-xl p-3">
                                                                ...
                                                                <p className="text-xs text-gray-400">Titular: <span className="text-white">{b.account_holder}</span></p>
                                                                                {b.cci && <p className="text-xs text-gray-400 flex items-center gap-2">Llave Interbancaria: <span className="text-white font-mono">{b.cci}</span> <button onClick={() => { navigator.clipboard.writeText((b.cci || '').replace(/[\s-]/g, '')) }} className="text-gray-500 hover:text-white"><Copy className="w-3 h-3" /></button></p>}
                                                            </div>
                ...
                <div className="bg-zinc-950 rounded-3xl shadow-2xl w-full max-w-lg mx-4 flex flex-col" style={{ height: '85vh' }}>
                  ...
                  <div className="px-4 py-2 bg-zinc-900 border-t border-white/5 shrink-0">
                    <p className="text-[10px] text-gray-500 text-center">
                      Si el pago fue exitoso, cierra esta ventana y serás redirigido automáticamente
                    </p>
                  </div>
                </div>
              </div>
